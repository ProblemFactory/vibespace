...
class TaskGroupManager {
  ...
  renderContext(id, { multi = false, ctxBase = null } = {}) {
    ...
    if (t.contextDir) {
      ...
      parts.push('', `## Task context folder`, '',
        ctxBase
          ? `\`${base}\` — a live-synced copy (newer file wins, ~1 min lag) of this group's shared workspace; every session of the group sees it.`
          : `\`${base}\` — the task's shared workspace, visible to every session of this task.`);
    ...
    if (t.contextDir) {
      parts.push('',
        ctxBase
          ? `Put findings, designs and artifacts that other sessions should see in \`${ctxBase}\` — it syncs back to the group's shared folder (newer file wins, within ~1 minute). Do not create a \`.vibespace/\` subfolder there.`
