...
class TaskManager {
  ...
  renderContext(id) {
    ...
    parts.push('', '### How to report back  (IMPORTANT — read this before using the tools)', '',
      ...
      `- \`vibespace-task status <active|paused|blocked|done>\` — the state of the WHOLE TASK.`,
      `- \`vibespace-task show\` — reprint the objective / checklist / activity log.`,
      '',
      '`vibespace-status` — report the state of THIS ONE SESSION right now. This is SEPARATE from the task status above — do not confuse the two (both have a "blocked"; here it means *this* session is stuck):',
      ...
      `- Keep it honest and current — the user reads it on the board to see which sessions need attention. Set \`blocked\` or \`needs-input\` (with a higher urgency) the moment you are stuck or waiting on the user.`);
    if (t.contextDir) {
      parts.push('',
  ...
  update(id, patch = {}) {
    ...
    if (patch.status !== undefined) {
      if (!STATUSES.includes(patch.status)) throw new Error(`status must be one of ${STATUSES.join('/')} (this is the WHOLE TASK's status; to report this session's own state use vibespace-status)`);
