...
const SETTINGS_SCHEMA = {
  ...
  'agents.vibespaceIntegration': {
    ...
    description: t('ON (default): sessions get the VibeSpace hooks (Task Group context, per-turn reminders, stop nudge), the vibespace-status/ask/task tools on their PATH, and passive usage capture. OFF: a pristine claude/codex — the hook registration is removed from ~/.claude/settings.json and ~/.codex/hooks.json immediately (restored on re-enable, unless you had removed the hook manually in Manage Agents), new sessions spawn with no VibeSpace env, tools or statusline, and already-running sessions stop receiving injected context, nudges and task reads. One residue: terminal sessions started BEFORE the flip keep their spawn-time statusline until restarted. The Ctrl+G editor, session persistence and remote transport are unaffected. Every option below only applies while this is ON.'),
