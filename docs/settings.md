...
| `window.mergeDropLayout` | enum | `tabs` | A window dropped onto another window's icon or tab bar: `tabs` = grouped as tabs (a toast offers "Show side by side"); `split` = shown side by side at once, the dropped window on the right, with **Undo** (it goes back where it was) and **Unsplit** (stay grouped as tabs, at the slot it was dropped on). A drop onto a group already side by side joins the half under the pointer, with **Undo**, under either value. Read at the drop |
| `window.openLinkPlacement` | enum | `split` | Where a file / folder opened from a chat (a path or a local link) appears: `split` = side by side with that chat window (on the side it is not on, if it is already side by side), `tab` = a tab right after it, `window` = its own window. The same path opened again from the same window shows the tab it is already in. The phone always opens its own window. Read when you open it |
...
| `desktop.idleTimeoutMin` | number | `0` | A desktop application window with no input for this long is stopped; `0` = never. "Keep running" in a window exempts that app. Stamped on each app at launch (changing it never touches a running app) |
...
| `claude.defaultPermissionMode` | enum | `''` | Default Claude permission mode for new or resumed Claude sessions. The dropdown (here and in the New Session dialog) reads as words with the protocol value as a hint — "Ask before acting · default", "Never ask (full access) · bypassPermissions", "Never ask — refuse what is not pre-approved · dontAsk" (`src/lib/permission-mode-labels.js`); the stored value is the protocol string |
...
| `claude.allowAgentTools` | boolean | `true` | VibeSpace's own agent tools run without a permission card for every step, in every permission mode: every claude spawn (chat and terminal, new and resumed, local and remote) carries `Bash(vibespace-browser:*)`, `Bash(vibespace-status:*)`, `Bash(vibespace-task:*)`, `Bash(vibespace-ask:*)`, `Bash(vibespace-docs:*)`, `Bash(vibespace-msg:*)`, `Bash(vibespace-window:*)`, `Bash(vibespace-channels:*)`, `vibespace-page` `list` / `kit` and the non-executing `vibespace-job` verbs (`list`, `show`, `poll`, `answers`, `logs`, `progress`, `ask`, `docs`, `stop`, `rm`, `notify`, `notify-cron`, `announce`, `subscribe`, `unsubscribe`) as `permissions.allow` in its inline `--settings`, plus — only when the session starts in a mode that asks anyway (`default`, `acceptEdits`, `plan`) — `permissions.ask: ['Bash(vibespace-page publish:*)']`, so publishing a page asks every time there even if your own settings allow it (an ask rule beats any allow rule); with full access (`bypassPermissions`) publish runs without asking as it always did, in `dontAsk` it is refused, and `auto` / no mode decide for themselves — our rules never change what the chosen mode does (the CLI's `flagSettings` layer — nothing is written to `~/.claude/settings.json`; a user's own inline allow/deny is kept, a user's own `--settings <file>` is left alone). Held — never pre-approved by these rules: `vibespace-job run` / `start` (they run any shell command), `vibespace-job access`, `vibespace-exit`, `vibespace-page publish`, and any other command in the same line; Always Allow on a held command remembers only that exact line (none for publish). The trade-off: anything the agent can read it can type or upload into a web page without asking — except an upload from `~/.claude`, `~/.ssh`, `~/.vibespace`, `~/.codex` or VibeSpace's account stores, which `vibespace-browser` refuses by name (`upload_secret_refused`); and a file WRITE (`download`/`pdf`/`screenshot`/`state save`/`record`/…) is confined to the project directory, `/tmp` or `~/Downloads` and refused elsewhere or into any of those stores (`write_path_refused`) — an allow rule on a tool is a grant on every path it can write, so the tool confines its writes. Off = every step asks again. Codex has no equivalent (its only per-command allow is an execpolicy file in `CODEX_HOME`; its default workspace-write sandbox already runs these tools without asking). Table + measurements: `src/agent-tool-rules.js` (lane L, 2026-09-25) |
