...
> **lane E（2026-09-25）—— 部分被取代（docs/design-desktop-apps-seamless §3.6，owner 的 D1–D7）。** D27 (a) 说"我们自己起的窗口都可寻址"，现在不是了：VibeSpace 起的窗口**在用户共享之前对所有 agent 隐藏** —— 共享给指定的 agent 会话或整个任务组，启动前（桌面应用对话框）或启动后（窗口的 ⋯ / 标题栏 / 任务栏菜单）都可以，随时可撤销；唯一例外是 agent 自己打开的窗口（只共享给它自己）。用户启动的桌面应用**浏览器**一经共享就与普通应用一样是窗口目标（它现在启动时就带无障碍开关：Chrome 带 `--force-renderer-accessibility` 时在默认预算内就能到达页面 —— 235–237 个节点、约 50 ms，按钮自己的 `press` 动作可用；不带时只有 4 个关闭的 frame）。每次共享都带**模式** —— auto | tree | pixels：pixels = `screenshot`（应用**自己的**窗口 —— 在 xpra 一级上，本节量过的根窗口截图是屏外合成的、是黑的）+ `click --at`（那张图像里的像素）/ `type` / `key` / `scroll`，树动词拒绝 `mode_pixels`；auto 在应用的树有可操作节点时解析为 tree，否则为 pixels。在 xpra 一级上，没人查看的窗口是未映射的 —— 像素全黑、XTEST 输入丢失 —— 所以那里每个像素动词都拒绝 `window_not_visible`。D27 (b)（tier 3，`window.realDesktopTargets` 后面的用户真实桌面）不变且正交。模型：src/window-reach.js；门禁 test-window-reach / test-window-targets / test-window-target §4。
...
> **lane E（2026-09-25）：** `list` 只列**共享**给调用会话的窗口（或它所在任务组的、或它自己打开的）—— 每行带模式（`mode: auto → tree|pixels (原因)`）；对未共享窗口的每个动词都是 `not_exposed`；像素共享拒绝 `snapshot` / `click @ref` / `type @ref`（`mode_pixels`）；`screenshot` 返回窗口自己的图像及其尺寸 + 原点 + 缩放，`click --at x,y` / `scroll --at x,y` 取的是这张图里的像素；新增 `scroll <h> up|down|left|right [--by N] [--at x,y]`；像素共享下不带 ref 的 `type`（以及对没有 EditableText 的可编辑字段）以按键输入；`window_not_visible` / `outside_window` 按名拒绝。手册（`vibespace-docs window`）都写明了；见 docs/design-desktop-apps-seamless §3.6。

> **lane E 核验（2026-09-25）：** 可达性 / 租约 / 暂停的判定在动词的每个 await 之后**重新判定**（引擎里的 `stillHeld`，紧接在每个动作和每次交付之前）—— 动词等待探测或 helper 期间落下的撤销、离开任务组、被接管或 tier 3 开关被关掉都会拦住它（`not_exposed` / `window_paused` / `not_attached` "ended while this verb ran"；不注入，不交出树或图像）。启动器的共享行现在写明一次启动会共享什么（应用记住的共享在卡片上有标签，启动后有提示）。**lane E 核验 low 级（未修复，2026-09-25）：** 像素原点跟着**最大**的窗口走（超大对话框会在截图和点击之间移动它）；30 秒唤醒节流只在内存里；「结束 <agent> 的占用」结束的是发送时的持有者；仅限人类的共享路由只检查 Authorization 头（认证关闭时省略令牌的本地调用者能通过）；`key ctrl+t` 后地址栏输入未跳转一次，未复现。修法见 docs/design-desktop-apps-seamless §3.6。

> **lane E 核验第二轮（2026-09-25）：** 重新判定按**调用点**钉住（引擎里每一个 `held()` / `heldOrUnlink(` / `stillHeld(` 调用单独删掉都必须让某个确定性竞态用例变红）；任务组存储**读不出来**时动词被拒绝 `reach_unreadable`（"the Task Group store could not be read — try again"，503）并**保留**租约 —— 绝不当成用户的撤销；接管或撤销现在会**取消**已经在执行的注入（`type` / `key` / `click --at` / `scroll`：按句柄杀掉租约自己的子进程，再经 XTEST 松开它按着的键 / 按钮 —— 实测：只杀不松会让键一直自动重复），动词回答 `window_paused` / `not_exposed` 并带 `did.partial`；按键键入的文本每次最多 1500 个字符（超时随之增长）；启动器对已经不在的主体用「当前未运行」/「当前不在列表中」说明，改过名的任务组用当前标题。**lane E 核验第二轮 low 级（未修复，2026-09-25）：** 未动过的「共享给 agent」弹出框显示空的选择器，而这一行说记忆会生效（L3）；「高级」/「最近」启动会套用记住的共享但点击前没有标签（L6）；40 个字符的中文 / 日文任务组名会把模式挤出卡片标签（L7）。修法见 docs/design-desktop-apps-seamless §3.6。
...
| **D19** | **一个 split 链里有第三个标签页时，点它会怎样？**（§4.6 —— 一个链可以有五个标签页而其中两个并排。） | (a) 它替换掉非 owner 的那一面；(b) 整个链翻回 `'tabs'`；(c) 开出第三面。 | **(a)。** 它保住绑定（聊天那一面 —— 浏览器**绑到**的那个东西 —— 原地不动），而且最不意外：变的是你原本没在看的那一面。(c) 按实测理由拒绝 —— 在大多数人实际使用的宽度以下，三个面全都不可用，而比例模型将不得不变成一棵树。(b) 会悄悄毁掉用户自己搭起来的布局。**2026-09-25 被取代（owner，inc-muhfb5al-jzk6 ——「我只能调整右侧展示的tab」；docs/design-split-ux.zh.md §8）：** 并排时每个标签属于一侧，标签条是两半；点一个标签就显示在**它自己**那一侧，两面都可切换（聊天那一面仍原地不动，除非点的是它那一半里的标签）。 |
...
| **D27** | **窗口目标能寻址哪些窗口？**（§4.9 —— 本机实测：X11 枚举看不见 Wayland 客户端，GNOME 的 Introspect 对我们 AccessDenied。） | (a) 只有**我们自己起的**（Xvfb+Xpra 里的应用）；(b) 加上用户真实桌面上的窗口，需要一次明确开启；(c) 都不做。 | **(a) 先发，(b) 作为一个带自己那次同意的明确开关。** (a) 那一列在 §4.9 的矩阵里全绿、不需要向任何人要权限、而且它是"agent 用一个原生应用干活"这个用例的全部 —— 用户桌面上那个正在被他打字的窗口不在这个用例里。(b) 的每一行都要标"这是你的桌面"，而它真正的门槛不是技术，是 §6.6 那条：对它的一次点击就是对用户真实会话的一次点击。 **lane E（2026-09-25）：** (a) 现在的意思是"VibeSpace 起的、且用户共享给了该 agent（或它自己打开）的窗口" —— 见 §4.9 的说明与 docs/design-desktop-apps-seamless §3.6。 |
