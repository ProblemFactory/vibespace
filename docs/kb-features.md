...
- **Chained popovers don't dismiss their parent**: `attachPopoverClose` (utils.js) ignores mousedowns whose target sits inside ANY `[data-popover]` — a context menu opened from a list row is a child interaction, not a dismissal (closing the list on right-click was reported as jarring). Opening a popover from regular UI still closes others (that mousedown lands outside any popover). The window list refreshes IN PLACE after a menu action via `onAction(kind)` (`render()`/`place()` split; re-placed because the height changes) — except `move`, which takes over the screen, so the list closes. **The close OWNS its one document listener (lane K verify r1, 2026-09-25):** `attachPopoverClose` registers it on an AbortController stored as `popover._closeCtl` (also returned as a disposer); a popover that can go by another path (a leave timer, Esc, a row) aborts it in its own cleanup, and a listener that finds its popover already gone removes itself on the next mousedown. Before, only an outside mousedown removed it — the taskbar's hover chooser, which opens and closes with no click at all, grew the document's mousedown listeners 3 → 33 over 30 hovers, each holding its detached chooser. The exclusion list is LIVE (`popover._closeExclude`): an owner that rebuilds its anchor pushes the new one (the taskbar group chooser's re-anchor), and the group chooser's rows follow the window list's right-click pattern — that tab's menu, the chooser pinned beneath and re-listed after the action.
...
- **VibeSpace's own tools do not ask per step; a card for them says what it does (lane L, 2026-09-25 — the naive-user study 2: 7–15 "Permission: Bash" cards per browser task, Always Allow never sticking, `close --all` feared).** Setting `claude.allowAgentTools` (Claude section, default ON): every claude spawn carries allow rules for vibespace-browser / -status / -task / -ask / -docs / -msg / -window / -channels / -page and the non-executing `vibespace-job` verbs in its inline `--settings` (nothing written to `~/.claude/settings.json`; `vibespace-job run` / `start` / `access`, `vibespace-exit` and any other command in the same line still ask). **Always Allow now sticks**: it reaches the CLI as `updatedPermissions` (it had been sent under a key the CLI drops — every Always Allow was a plain Allow), and on a card for one of our tools it allows the whole tool (the CLI suggested only the verb, `vibespace-browser click *`); its tooltip names the rules. **A browser-only line wears the agent browser's face** — its glyph, "Agent browser", the steps in words ("Open https://…", "Click @e1 · Read the page") on the card head and in the permission prompt ("Permission: Agent browser"); a line that also runs something else stays a Bash card that says what our part does, "also runs: pgrep", and for `close --all` "Closes only this conversation's browser — browsers of other sessions are not touched." A hovered Allow stays green (it used to turn into a dark button with dark words that read as disabled — the "needs two clicks" report). src/agent-tool-rules.js is the table + the words; gates test-agent-tool-rules / test-new-session-dialog. **r2 (the lane's adversarial verify, 2026-09-25) — the verb tables and the trade-off:** allowed whole — vibespace-browser, -status, -task, -ask, -docs, -msg, -window, -channels; verb-listed — `vibespace-job` (list, show, poll, answers, logs, progress, ask, docs, stop, rm, notify, notify-cron, announce, subscribe, unsubscribe; HELD run / start / access) and `vibespace-page` (list, kit; **publish = ASK**: owner decision 2026-09-25 — it is not in the allow list and the spawn also carries `permissions.ask: ['Bash(vibespace-page publish:*)']`, which beats any allow rule — see the r3 mode table below); held whole — vibespace-exit and the non-agent helpers. **Always Allow on a held verb or tool** allows only that exact command line (the CLI would have saved `vibespace-job run *` — every future shell command through that verb), and where no exact line can be saved (a compound, a `*` in the line) or the verb is an ask verb, the button is not shown and the card says why ("Publishing a page asks every time — Always Allow is not offered."). **The browser face is worn only by the bare tool name** — `./data/bin/vibespace-browser …`, `/tmp/x/vibespace-browser …` or `PATH=… vibespace-browser …` is some other file, so its card stays a Bash card that names it ("also runs: /tmp/x/vibespace-browser"). **The accepted trade-off, stated in the setting — the READ and WRITE twins:** anything the agent can read it can type or upload into a web page without asking — except an UPLOAD (the READ twin) from ~/.claude, ~/.ssh, ~/.vibespace, ~/.codex or VibeSpace's account stores (data/subs, data/codex-subs), which `vibespace-browser` refuses by name (`upload_secret_refused`, before the binary or the server; a symlink into a store counts). **r4 (the round-2 verify's surviving MAJOR) — the WRITE twin:** an allow rule on a tool is a grant on every path the tool can WRITE, and every writing verb (`download`/`pdf`/`screenshot`/`state save`/`record`/`trace`/`profiler`/`network har`, the `-o`/`--screenshot-dir` flags — censused from the CLI help) confines its output to the session's project cwd, the OS temp dir or ~/Downloads and refuses anything else — or any credential store — by name (`write_path_refused`, before /resolve and before the binary; measured red on 0.38.1: `download`/`pdf` wrote `~/.ssh/authorized_keys` and `~/.claude/settings.json`, `open data:text/html,<a download>` reaches the payload with no external server). The invariant: an allow rule on a tool is a grant on every path the tool can write; the tool confines its writes or the rule cannot exist. **r5 (the round-3 verify's CRITICAL) — THE FRAME RULE:** a path is judged and written in the SAME frame. The browser's daemon resolves a relative word against ITS OWN cwd (the launcher's — the keeper's used to be the server's, the checkout), so the CLI never hands it a relative word: every path slot (exact, measured per verb — `download`'s word after the selector, a lone `screenshot` word only when it holds `/` or an image extension, `record start`'s path never its URL, the upload/`state load`/`--baseline`/`--init-script`/`--curl` reads too) is resolved PHYSICALLY (a symlink before any `..` after it; `~` expanded — the binary never does) from where the agent typed it and handed over ABSOLUTE, in the argv, in every batch line (an argument batch goes over as the binary's stdin JSON) and every stdin element; and every daemon runs in a private `<tmp>/vibespace-browser-cwd-<uid>` (the keeper's runtime and the CLI's own spawn — never the checkout, refused by name). The fence is the SESSION's directory — `VIBESPACE_SESSION_CWD`, exported by every spawn path (the local wrapper argv, the remote prelude) — never the shell's cwd (absent ⇒ only `/tmp` and `~/Downloads`); a project inside a dot-directory writes under its own root; `.ssh`/`.claude`/`.codex`/`.vibespace`/`.git` anywhere and VibeSpace's own data dir are refused wherever they stand; `AGENT_BROWSER_SCREENSHOT_DIR` is not passed. The job cards say what they do ("Stop or remove one of its background jobs", "Schedule a reminder that wakes this conversation", "Let others see or control one of its background jobs"). With a user's own `--settings <file>` and effort ultracode there is still exactly one `--settings` flag — the user's file; the ultracode mode stands down and the server journal says so. **r3 (integrator decision 2026-09-26) — the publish ask rule follows the SPAWN mode.** On 2.1.281 an ask rule is checked BEFORE the permission mode, so r2's rule on every spawn made `vibespace-page publish` prompt in a full-access session that had never asked. The law: **our injected rules never change what the user's chosen mode does — they only keep OUR allow list from widening.** So the ask rule rides only a spawn whose mode asks anyway, and the allow list is verb-listed (list, kit — never publish) in every mode:

  | spawn mode (`--permission-mode`, read off the argv) | `permissions.ask` publish rule | what `vibespace-page publish` does |
  |---|---|---|
  | `default` / `manual` (2.1.281's alias of default) | carried | asks every time — even when the user's own settings hold a broader allow rule |
  | `acceptEdits` | carried | asks every time |
  | `plan` | carried | asks (plan mode asks for commands anyway) |
  | `bypassPermissions` ("Never ask (full access)"), or `--dangerously-skip-permissions` in the session's own args | NOT carried | runs without asking, as it did before lane L |
  | `dontAsk` | not carried (nothing to add) | refused — it is not pre-approved, exactly as it always was |
  | `auto` | not carried | the mode's own safety check decides, as before lane L |
  | no flag ("Default (Claude Code's own setting)") | not carried | the CLI's own mode decides (a user's `defaultMode`, bypass included, or 2.1.281's auto fallback — unknown at spawn); in its default mode publish asks because it is not in our allow list |

  The mode is `spawnPermissionMode(args)` over the argv the CLI receives: `--dangerously-skip-permissions` wins (2.1.281 `Rvr` makes it the first candidate), else the LAST `--permission-mode` (a user's own flag in extraArgs lands after the session's), else unknown. **The rule follows the SPAWN mode:** a mid-session mode switch (`set_permission_mode`) carries the mode alone and rewrites no settings (no `apply_flag_settings` the product sends names `permissions` — census-pinned), so a session started in `default` and switched to full access still asks for a publish until its next spawn (the mode that spawn is given decides again), and one started in full access and switched to `default` asks for a publish anyway (not allowed) unless the user's own rules allow it. A user's own inline `ask` is kept verbatim in every mode. Gate: test-agent-tool-rules ⑬ (every mode, the argv overrides, the switch, a patched copy that always injects goes red on the bypass leg).
...
- Command mode: `Ctrl+\` prefix key (tmux-style), [CMD] indicator in taskbar, 2s auto-exit. **`v`** = side by side on / off for the active window's tab group (active tab left, most recently used other tab right, undoable), **`V`** = swap left and right; without a group of ≥ 2 tabs a toast says "Group two windows first" (never silent); the armed hint lists `v split`. **`{` / `}`** (split tabs v2, 2026-09-25) move the active tab one place left / right within its own list (its half in a split; stays in command mode) — the same verbs as the registry chords **Ctrl+Shift+PageUp / PageDown** (app.js; inert outside a ≥ 2-tab group, a terminal passes them through only in a group; a regular browser tab may keep the chord for itself)
...
- Tab groups: drag window icon onto another window's icon to merge into Chrome-style tab group. Tab bar with rounded top tabs, active tab connects to content. Drag tab downward to pull out (follows cursor with snap). Close individual tabs. Tab chain data synced across clients via layout-sync. **Side by side (split UX, 2026-09-23, docs/design-split-ux.zh.md):** the tab merge is the ONE exception to ordinary drag-and-snap — no drag ever splits. After a merge the strip's two-column button pulses once and a 5 s toast offers "Show side by side"; the button (active tab left, the most recent other tab right) is the entry, and in a split the same button is the badge (Unsplit / Swap left and right; the divider's right-click has the same two). The strip is drawn in visual order with a bar between the pane tabs and an owner-colour underline on each; every user-initiated split shows "Side by side: A | B · Undo" for 5 s. Right-clicking a tab opens THAT tab's window menu. **Split r1:** the button's tooltip is re-written on every tab switch (it names the partner the click will use — on a ≥ 3-tab group the most recent other tab changes with each switch); a split taken any way withdraws the post-merge "Grouped as tabs" toast, and its button, if it still runs after the group changed, says "Already shown side by side" / "The tab group changed — nothing to show side by side" (never a silent no-op). **Split tabs v2 (2026-09-25, inc-muhfb5al-jzk6, docs/design-split-ux.zh.md §8 — the owner: tabs all on the left, no reorder, only the right side switchable):** in a split every tab belongs to a SIDE and the strip is TWO HALVES laid on the panes' own columns (a subgrid of the host — they follow the divider live), the glyph over the divider, each side's shown tab highlighted; a tab of the left half shows on the LEFT, of the right half on the right (D19 (a) retired). A horizontal tab drag REORDERS (marker, Esc cancels; the first 8 px decide, vertical = the old pull-out); across the middle it MOVES the tab to the other side (a shown tab stays shown there, the side it left shows its neighbour; the last tab out ends the split) — never a new split. A merge drop lands at the slot under the pointer (in a split: that half). A press inside a pane focuses that pane. A tab GROUP dragged by its title bar only moves (it used to orphan its other tabs when dropped on an icon). Settings → Window: **`window.mergeDropLayout`** tabs (default) | split — the merge lands side by side at once, the dragged window on the right, toast **Undo** (back where it stood) / **Unsplit** (back to the slot it was dropped on — v2 verify r1 ③); a drop onto a chain ALREADY split (either setting) joins the half under the pointer, shown there, with a "Side by side: A | B · Undo" toast (the window back where it stood, the split back to its sides / order / pair before the drop — r1 ④); a pointer drag first SHOWS the tab (a press shows it), so a tab dragged across arrives shown; the right column never renders narrower than the controls + a tab's room and the ⫿ badge steps aside on a tight column (r1 ②); a divider drag survives another client's structural change made during it (held locally, re-sent once — r1 ①; kb-design-lessons §6b item 7); **`window.openLinkPlacement`** split (default) | tab | window — a path / local link opened FROM A CHAT is born beside THAT chat (its own window, never the active one; on a chain already split the side the chat is not on), the same path again shows its tab (a `:line` link moves the editor), the phone keeps its own window. Other clients apply a same-member change IN PLACE (no re-parenting); a member still being replayed asynchronously keeps its chain pending (no free viewer that would break the group on the next save).
...
- Taskbar tab-group stacking (Windows-style): a tab-group host renders ONE stacked item (`_buildGroupItem`/`_buildStackIcon` in taskbar.js) — the unique tab icons offset like a card stack (active tab frontmost; a lone icon gets a faded ghost behind), a count badge, titled by the active tab + "N windows grouped". Click → ACTIVATE the group (lane K, 2026-09-25, owner "这个体验比较差" — the old click was always the chooser): restore + focus + raise its active tab through `activateWindow`, the single button's own path; a click while the group is IN FRONT (host or a tab focused, unminimized, on this desktop — PURE `inFrontOf`) = the `showTabGroupList` chooser (click one → restore+focus group + `switchTab`); HOVER with 300 ms intent on a mouse or a pen (never touch / a drag / another popover) = the same chooser, non-modal, closing 250 ms after the pointer leaves button + chooser — armed by the pointer MOVING over the button once per visit, a press spending the visit until the pointer leaves, the visit kept per group so a taskbar rebuild under a resting pointer (a tab title change) grows nothing (2.369.184, PURE `hoverStep`); Enter/Space = the click rule, ArrowUp = the chooser. Decisions PURE in src/lib/taskbar-group.js (gates test-taskbar-group + test-taskbar-group-ui). The chooser's ONE cleanup (a MutationObserver, whichever path removed it) also aborts createPopover's outside-click close (`pop._closeCtl`) — verify r1's minor finding, pinned by test-taskbar-group-ui (j) (30 hover cycles, document mousedown listeners unchanged; CONTROL 2 = the disposer reverted ⇒ +30) and test-taskbar-group §7/§9. Right-click on the button acts on the whole group; right-click on a chooser ROW = that tab's window menu (the window list's row pattern), the chooser pinned beneath and re-listed from the live chain after the action (a Move closes it). **The five verify-r1 lows, fixed in the same lane (2026-09-25; each reproduced red on a scratch server with a real mouse, now a red→green leg with a control):** ① a reload shows a restored tab group as ONE grouped button with no input — tab-group.js `restoreTabChain` ends with `_notify()` like every other chain mutation (before: separate single buttons until the first window event, so the first click after a reload landed on a single button); ② a taskbar rebuild under an open chooser (a tab title change re-keys the structure) RE-ANCHORS it — `_buildGroupItem` hands the new button to `pop._chooser.reanchor()`: `aria-expanded="true"`, the outside-click close's live exclusion list, and Esc focus / the cleanup's reset / placement read the live anchor (before: the rebuilt button said "false", Esc dropped the focus on body, a click on it closed and re-created the chooser); ③ a PEN hovers like a mouse (`pointerType` mouse or pen with `(any-pointer: fine)`; touch never); ④ right-click on a chooser row (above; before: the browser's native menu); ⑤ the dead `.taskbar-window-list` (nothing produces it — the window list is `overlap-switcher`, a [data-popover]) is gone from `_otherPopoverOpen` and from app.js's autohide conceal guard, and test-taskbar-group §7's PRODUCER CENSUS keeps every class those two guards query produced by the tree, each guard's pre-fix spelling as its control. Gates: test-taskbar-group-ui (k)–(n) + CONTROL 2 (the fixes reverted ⇒ each red), test-taskbar-group §7 pins + the census + §9 `liveExclusion`. active/minimized/waiting is group-aware (`_applyTaskbarItemState`, reads `dataset.groupTabs`). Previously only the host showed and every guest silently vanished from the taskbar.
...
- **The New Session dialog starts clean (lane L, 2026-09-25).** The Session Name field is emptied on every open (the study: "S11 chromeS12 first…"); the other prefills below stay (the cwd passed in or last typed, the Task Group, the harness defaults). Escape closes only what is open inside the dialog first — the cwd suggestion list, a native dropdown's open picker — and the next Escape closes the dialog. The Permission Mode options read as words with the protocol value as a hint ("Ask before acting · default", "Never ask (full access) · bypassPermissions"), zh + ja; the stored values are unchanged.
...
THIRD tab of a split chain replaced the non-anchor pane in place (D19 (a)) —
**superseded 2026-09-25 (split tabs v2):** every tab belongs to a side and a
click shows it on ITS side, so the chat pane changes only when a tab of its
own half is clicked. **Lifecycle, three paths, one
...
unchanged, a third tab joining the right side and shown there (split tabs v2 —
D19 (a) retired), closing the chat host (the only tab of its side) ending the
split with no dangling id,
...
### One session, many browsers — the Agent browser window's strip, the per-conversation limit, pop-out / fold-back, grouped chats that follow (lane P, 2026-09-26; docs/design-browser-multiview.zh.md §2 / §3 (b) / §4, D3 / D4 / D5)

**One window, every browser of the session.** The Agent browser window's strip is PURE `browserListFor(status)` over the session's status answer: its attached profiles, its OWN browser (the managed ephemeral record — "This session", full title "This conversation's browser") and each HELPER's browser (a sub-agent's `new-child` handle — "Helper: <its Task's description>" when a witness named it, else "Helper N"). Nothing else ever becomes a tab. Each tab: a state dot (pulsing green = a command is running in the pane you watch; solid = running; HOLLOW = released — "the next command starts it again", or ended), the name (≤16 characters, the full name in the tooltip), who is driving (`agent` / `you`), and a profile's owner dots. It shows with ≥2 browsers (one browser keeps the old look), or with one when the conversation is AT its limit. A browser that starts later becomes a tab AT THE END of the strip (each window keeps its own first-seen order) and NEVER changes the pane you are watching — not even while you drive it (same socket, same takeover). Narrow windows fold the strip by priority into `▾+N` (the tab you look at never folds; a tab running a command outlasts a quiet one; right to left). A helper's tab shows the helper's OWN picture (its own recorded pairs, never the parent's); taking it over pauses THAT helper (`browser_paused` keyed on its handle), never the parent.

**The limit is the conversation's (D4).** The chip at the strip's end reads `own/cap` — this conversation's RUNNING browsers (helpers count) against ITS limit: explicit (the chip's −/+ 1..6, Session Properties → Browsers at once; kept per conversation in the keeper's registry, so a resume keeps it) > its Task Group's default (task detail → Browsers per conversation — for the group's NEW conversations: stamped when a conversation starts, a running one keeps the default it started with, lane P verify) > Settings → Agent browser → *Browsers one conversation may run at once* (3). Red at the limit. Clicking it lists only this conversation's running browsers (never one you are driving), each with Stop — a profile another conversation also uses says "detach it instead" — and the limit stepper (+ *Use the default*). The agent's next browser past the limit is refused `browser_cap` with `scope:'conversation'`, the sentence about this conversation only and the remedy naming the chip and Session Properties. The machine's ceiling (six, shared with desktop apps) stays the hard top: its refusal says "machine ceiling reached", names only the asking conversation's own browsers and COUNTS the rest ("N are in other conversations or desktop apps") — never another session's name, label or lease id. The chip's popover says "Machine ceiling reached" on its own line when the machine is full.

**Released after the turn (B-325a).** A conversation's own browser and its helpers' are stopped 3 minutes after its turn ended (Settings → Agent browser → *Release a conversation's browser after its turn ends*, 0 = never, 30 s floor) — never while the user drives it, never while a live view watches it, never with an unknown turn (**a TERMINAL-mode session publishes no turn at all ⇒ unknown ⇒ its browser is kept until the CLI's own idle timeout — only CHAT sessions are released after the turn**, lane P verify r2). The record and its lease stay, the tab turns hollow, the chip drops by one; the next page verb starts it again through `ensureEphemeral`. A live view NEVER starts a browser of any kind (lane P verify, 2026-09-26): a released ephemeral / helper browser and a stopped attached profile say "Released — the next command starts it again" (a released OWN browser while a profile is attached: "Released — used again only when no profile is attached" — a bare command lands on the attachment); a conversation that has not opened its browser yet says "Not started yet — the next command starts it" (the keeper asks only the launch-free `session info` — `stream status` would spawn a daemon); the pane is picked up by itself when it runs again; one picked up while it is still STARTING waits for the start (a start that fails is what the pane says — lane P verify r2). A conversation's browser is started with the idle timeout its spawn pairs name (the setting only when they name none), so neither the agent's first command nor a live view replaces it after the setting changed (measured on 0.38.1: a client with a different idle restarts the daemon).

**Pop out, fold back (D3).** No new window type: a session may have N `browser-live` windows, all showing the same strip, each with its own selection. Right-click a strip tab → *Open in new window* = one more window preselecting that browser. Fold back: the window's title-bar menu (also its tab / the phone's long-press) → *Fold back into the Agent browser window* (the main window SELECTS the tab it showed and gets the focus; a main window you are DRIVING keeps its browser — said in a toast — and the folded one stays in its strip; folding back the window you are driving CARRIES your control to the main window first — a `pass` from the holder to the other view of the same browser — so nothing is handed back to the agent and nothing is announced; driving in both is refused in words, lane P verify), or drop its ICON on the other window or on the group holding it: the one drag exception folds back instead of grouping two views of one session (every merge drop — icon, tab, title bar — goes through the same rule). The MAIN window is `win-blive-<session>` while it is open, else the oldest other one — closing the main one leaves the next one as the main one.

**Two grouped chats, each with its browser (D5).** Still two panes, never four columns. A new browser's live view is born beside its chat only while that chat is NOT yet side by side (one pane ⇒ chat | browser); a chat already side by side gets its browser as a quiet TAB (on the side that already holds a browser, else on the chat's own side) that pulses once — the shown pair, the window's place and size do not move. In a group with the chats on one side and their browsers on the other, showing a chat makes the other side show ITS browser, and showing a browser makes the other side show its chat — only when the other side currently shows a partnered tab of that kind; a chat you placed there (two chats side by side), a file, a browser whose chat is on the same side, or an ended session: nothing follows. Both live views are already connected, so the follow re-displays and never reconnects. The phone shows one pane and never follows. The follow runs on the client that clicked; the other clients receive the resulting layout.

Gates: test-browser-handles (fast, 152), test-live-strip (fast, 36), test-browser-ephemeral (fast, 139), test-chain-layout (fast, 83), test-browser-pin (181), test-contributions (219), test-session-schema (7); heavy test-browser-live (116), test-browser-multiview (29), test-split-ux / test-window-binding green.
