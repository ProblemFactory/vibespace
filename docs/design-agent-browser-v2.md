...
> **Rename note (takeover T5, 2026-09-24, direction B — docs/design-browser-faces.md):** this section's live-view window (`browser-live`) is labelled **Agent browser** in the UI (Agent 浏览器 / エージェントブラウザ); the toolbar's embedded browser is **Web view**; the desktop-app browser card is sub-labelled **Browser app**. Only the displayed names change — the window type, openSpec, settings keys and rail id do not. See §11a.
...
> **Lane E (2026-09-25) — superseded in part (docs/design-desktop-apps-seamless §3.6, the owner's D1–D7).** D27 (a) said "every window we started is addressable". It no longer is: a window VibeSpace started is **hidden from every agent until the user SHARES it** — with specific agent sessions or whole Task Groups, before launch (the Desktop apps dialog) or after (the window's ⋯ / title-bar / taskbar menu), revocable at once; the one exception is a window an agent opened itself (shared with that agent only). A desktop-app **browser** the user launched is a window target like any app once shared (it now starts with its accessibility switch: Chrome with `--force-renderer-accessibility` reaches the page in the default budget — 235–237 nodes, ~50 ms, the button's own `press` action works; without it 4 closed frames). Every share carries a **mode** — auto | tree | pixels: pixels = `screenshot` (the app's OWN windows — on the xpra rung the root grab this section measured is composited offscreen and black) + `click --at` (a pixel of that image) / `type` / `key` / `scroll`, the tree verbs refused `mode_pixels`; auto resolves to tree when the app's tree has a usable node, else pixels. On the xpra rung a window nobody is viewing is unmapped — its pixels are black and XTEST input is lost — so every pixel verb is refused `window_not_visible` there. D27 (b) (tier 3, the user's real desktop behind `window.realDesktopTargets`) is unchanged and orthogonal. Model: src/window-reach.js; gates test-window-reach / test-window-targets / test-window-target §4.
...
> **Lane E (2026-09-25):** `list` shows only windows SHARED with the calling session (or one of its Task Groups, or opened by it) — each with its mode (`mode: auto → tree|pixels (why)`); every verb on an unshared window is `not_exposed`; a pixel share refuses `snapshot` / `click @ref` / `type @ref` (`mode_pixels`); `screenshot` answers the window's own image with its size + origin + scale and `click --at x,y` / `scroll --at x,y` take a pixel of it; NEW `scroll <h> up|down|left|right [--by N] [--at x,y]`; `type` without a ref under a pixel share (and into a field editable without EditableText) types keys; `window_not_visible` / `outside_window` by name. The manual (`vibespace-docs window`) says all of it; see docs/design-desktop-apps-seamless §3.6.

> **Lane E verify (2026-09-25):** the reach / lease / pause decision is RE-ASKED after every await of a verb (`stillHeld` in the engine, immediately before each act and each hand-over) — a revoke, a Task Group left, a takeover or the tier-3 switch going off that lands while a verb awaits a probe or the helper stops it (`not_exposed` / `window_paused` / `not_attached` "ended while this verb ran"; nothing injected, no tree or image handed over). The launcher's share row now names what a launch shares (a card chip for an app's remembered share, a toast after it). **Lane E verify lows (not fixed, 2026-09-25):** the pixel origin follows the LARGEST window (an oversized dialog moves it between screenshot and click); the 30 s wake pacer is in memory only; "End <agent>'s hold" ends the holder at SEND time; the human-only share routes check the Authorization header only (auth off ⇒ a token-less local caller passes); one unreproduced omnibox-typing miss after `key ctrl+t`. Each with its proposed fix in docs/design-desktop-apps-seamless §3.6.

> **Lane E verify r2 (2026-09-25):** the re-check is pinned PER SITE (every `held()` / `heldOrUnlink(` / `stillHeld(` call of the engine removed alone must turn a deterministic race leg red); a Task Group store that cannot be READ refuses the verb `reach_unreadable` ("the Task Group store could not be read — try again", 503) and KEEPS the lease — never read as the user's revoke; a takeover or a revoke now CANCELS an injection already running (`type` / `key` / `click --at` / `scroll`: the lease's own child killed by its handle, then the keys / buttons it held released through XTEST — measured: a bare kill leaves the key autorepeating) and the verb answers `window_paused` / `not_exposed` with `did.partial`; typed-as-keys text is at most 1500 characters a call (the timeout scales with it); the launcher words a remembered principal that is gone ("not running now" / "not in the list now") and a renamed group by its current title. **Lane E verify r2 lows (not fixed, 2026-09-25):** the untouched "Share with agents" popover shows an empty picker while the row says the memory applies (L3); an Advanced / Recent launch applies a remembered share with no chip before the click (L6); a 40-character zh / ja group name clips the mode off the card chip (L7). Each with its proposed fix in docs/design-desktop-apps-seamless §3.6.
...
| **D27** | **Which windows may a window target address?** (§4.9 — measured here: X11 enumeration cannot see Wayland clients, and GNOME's Introspect answers AccessDenied to us.) | (a) only the ones **we started** (applications under Xvfb+Xpra); (b) plus windows on the user's real desktop, behind an explicit switch; (c) neither. | **(a) first, with (b) as an explicit switch carrying its own consent.** Column (a) is green throughout §4.9's matrix, needs nobody's permission, and is the whole of the "an agent works in a native application" use case — the window the user is typing in right now is not in that use case. Every (b) row must be marked "this is your desktop", and its real barrier is not technical but §6.6's: a click there is a click in the user's real session. **Lane E (2026-09-25):** (a) now means "the windows VibeSpace started AND the user shared with that agent (or it opened itself)" — see §4.9's note and docs/design-desktop-apps-seamless §3.6. |
...
## 11a. The takeover (2026-09-24 — docs/design-browser-takeover.md is its authoritative design)

> The owner (2026-09-24, verbatim): "那就选B吧 我还是觉得浏览器操作有点奇怪，另外建议vibespace完全接管浏览器工具，不要再给agent留agent-browser单独的入口了，也不要兼容agent直接操作agent-browser的行为。最好就是直接从系统path隐藏agent-browser，也修改一下web-access这个skill。" — take direction B; VibeSpace takes the browser tool over completely: no separate `agent-browser` entry for agents, no compatibility with agents driving it directly, hidden from the PATH, the web-access skill rewritten.

Numbered 11a so the anchors of §11 (D1–D35) and §12 stay put. It rewrites several premises of the "two doors" era in this design:

* **One door (T1/T2, chunk 1).** `vibespace-browser <verb>` takes every page verb directly (§5's `-- <args>` survives only as the valve for a newer verb), classified by the PURE `src/browser-verbs.js`; raw CDP, identity/launch flags and the user's verbs are refused by name locally. `agent-browser` on a session PATH is a shim that refuses and never forwards; the remote prelude's order is fixed to finder → tools. The teaching lines (§5) never name the hidden CLI (test-architecture §52).
* **The managed ephemeral browser (T3, chunk 2 — replaces §3.2's "nobody watches the ephemeral browser").** A conversation's first page verb with no attachment makes the keeper record an `ephemeral:true` record (owned by the conversation, ns = `vs-<browserKey>` — P0's four session variables ARE its identity, kept by D1) plus a lease aliased `ephemeral` that is never an attachment; the keeper starts it (or adopts one already running) under the session's EXACT spawn pairs, NEVER re-running the browser-env ladder. It counts against `CONCURRENT_CAP` (the one shared with desktop apps, D2; the seventh conversation's first verb gets `browser_cap` by name, naming every holder and the two ways out), is sampled by the runaway guard, an idle-out is recorded `stopped` (not an error), a restart adopts it; when the conversation ends the lease drops, the browser stops and THE RECORD REMOVES ITSELF (a named profile never does). The live view's port, the takeover key `<bk>|ephemeral` and the trace scope `ephemeral` are unchanged (port parity is gated). It is `sharing:'owner'` and not CDP-mediated: during a takeover it is stopped at `/resolve` with `browser_paused`. Remote sessions (rung H, D8) and instances with `browser.isolateSessions` off (the shared rung, D7) stay UNMANAGED, said so in the manual.
* **The attach gap (T6).** A browser window launched as a desktop app (a record or registry row carrying `browser` OR `category: 'browser'` — the latter is the only marker this tree's `DEFAULT_REGISTRY` has, the r1 correction; an ad-hoc launch of a browser row's executable too) is the user's own window: `vibespace-window attach` / snapshot / act / screenshot / watch refuse it `browser_is_human` by name, and `list` omits it.
* **The §4 rename (T5, chunk 3, direction B).** See the note at the head of §4: the three browser faces get their own names — Web view / Agent browser / Browser app; not one id, openSpec, settings key or rail id moves.

* **r1 (after the adversarial verify).** A flag between a verb and its noun hides nothing (`get --json cdp-url` is refused too); the env twins of every refused flag no longer reach the real binary through `vibespace-browser` (the child env is built from the server's `spawnEnv` and `/resolve`'s pairs; an `AGENT_BROWSER_*` key the agent added is said by name and dropped); the route error strings no longer teach the hidden CLI (test-architecture §52b widened to the route modules' strings). Authoritative text: the takeover design's §3.3 r1 addendum and §8.
* **r2 (the second adversarial verify).** Only the NOUN of a `get` decides (`get attr @e cdp-url` reads an attribute named cdp-url and runs; a boolean flag's optional `true`/`false` is skipped, so `get --json true cdp-url` stays refused — measured on the real binary); the socket root is identity — `/resolve` names the root the keeper runs the browser under and `vibespace-browser` sets it last, so an exported `AGENT_BROWSER_SOCKET_DIR` / `XDG_RUNTIME_DIR` cannot point a command at a daemon the keeper does not see; a desktop-app browser is recognised by its exec name, its launcher's program, its app id or its running process (Chrome, Edge, Brave, flatpak Chromium …), not only the registry's two rows; §52b scans every tracked agent CLI and the window / trace route modules. Authority: docs/design-browser-takeover.md §3.3 r2 addendum, §8 r2.
* **r3 (the third adversarial verify).** The router's global-flag tables are MEASURED off the binary (`get --idle-timeout 5m cdp-url` leaked: that flag is documented only as an env var), `get` passes only a noun it reads, and a flag of unknown arity is judged under both readings; the config FILE is named, never searched — the keeper runs every browser with a named file, `/resolve` names it, `vibespace-browser` sets it last (or composes one by the same rule) — and a PROJECT `agent-browser.json` only narrows (its launch keys are no longer layered into the generated config: this withdraws half of §3.2.2's r3 "layer it the way the CLI does"; the fence half stays), while no raw-debugging switch reaches a browser from any file. Authoritative text: the takeover design's §3.3 r3 addendum and §8.
* **r4 (the fourth adversarial verify).** Every browser the binary launches listens on a random `--remote-debugging-port=0` (measured), so §3.2.2's "no raw-debugging switch survives" means a FIXED port; what keeps the endpoint off the sanctioned road is that `vibespace-browser` refuses a navigation off the web (`open chrome://version` / `open file://…/DevToolsActivePort` printed it; `local_scheme_refused`, `state load` of such an origin too), composes "the machine's own configuration" from the ACCOUNT's passwd home (never `$HOME`), hands a stdin `batch` over in the binary's own JSON form, and trusts its measured flag table only on the version it was measured on. docs/design-browser-takeover §3.3 r4 / §9.

Gates: test-browser-verbs, test-remote-shell, test-architecture §52 (chunk 1); test-browser-ephemeral, test-browser-mediation ⑤, test-window-targets §4's T6 legs, test-browser-housekeeping's "Ephemeral browsers" section (chunk 2).
