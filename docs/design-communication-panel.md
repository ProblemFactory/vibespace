...
> **SHIPPED update (2.369.165, integrations chunk 2 — `docs/design-integrations-per-account.zh.md` r4 is authoritative):** the lark / gmail / fake rows are `bindsPerAccount` — their OAuth client is chosen PER ACCOUNT where the account is added (the storage mount dialog's `OAuth client` grammar), never on an Integrations card: `GET /api/integrations` lists the six browser rows only and GET / PUT / DELETE / test on the three answer `404 binds-per-account` by name; the `own` rung is retired for them (`own-retired`); the choice lives on the account record (`cluster:<k>` unchanged, or `custom` + `credential {appId, appSecretEnc}` sealed under `.channels-key`); presets `{key,label}` come from the one env reader both features share; a legacy `own` account's values are copied onto it once (reader-side + migration `2026-09-channel-custom-client-inline`), integrations.json's old values stay in place. The lark / gmail cards, radios / dropdown and Test buttons described below are the SUPERSEDED r0 shape; the six browser rows are unchanged.

> **SHIPPED update (2.369.165, integrations chunk 3 — the client):** the Accounts part ends in ONE `Connect an account` button (the type-first storage dialog — the shared `src/lib/mounts-dialog.js` component; the dialogs themselves live in `src/lib/channel-account-dialogs.js`); an account is a credential-first card (the state dot, the client chip, a health line in the storage detail-line grammar, the storage `.mounts-errline` sentence + the why code + `Re-authorize {product}…` when the sign-in died, the "Login only" wording + Track… for an account tracking nothing, tracked conversations as ↳ child rows), its ⋯ in the storage row's order (D6); Edit (the custom secret prefilled; switching the client opens Re-authorize) / Duplicate / Re-authorize (the storage re-authorize dialog + the client select on top) / Remove refused by name. The connect wizard, its three copy paths, its own flow dialog, the credential step and "Add account…" described below are DELETED.

> **SHIPPED (2.369.165, integrations chunk 4 — r4 shipped whole):** the storage side adopted "switching the client IS a re-authorization" too (D2, its own commit: switching a Drive / Gmail connection's OAuth client in Edit saves the other fields and opens Re-authorize under the new client; the token lands with the client); `scripts/test-oauth-field-parity.mjs` census-pins everything the two features share (one component, the shared spellings asserted on both sides, re-authorize, Edit's button order, where Remove lives, D8's borrowed sentence, the §4 i18n key list parsed from the design doc itself). This section's configuration UI is `docs/design-integrations-per-account.zh.md` r4 (SHIPPED); the Integrations window keeps the six browser rows only.
...
## 22. The owner's clarification (2026-09-22): the group list first — this is a simple IM, not "a panel of agent information sources"

> English twin of the Chinese §22 (docs/design-communication-panel.zh.md), which
> was written first; the owner's words are quoted in the original.

The owner (excerpt): "我不太需要一个 agent 订阅另一个 agent 的消息作为信息源这种 feature … 应该是一个'顺便'功能, 比如 'message watcher — lark/gmail/agent', 而不应该在界面里放在第一位。我设想的是第一位看到的是'群列表', 所有的群都至少有两个成员 (agent channel: 至少俩 agent + 我作为虚拟观察者, 我能通过面板发消息进去, 俩 agent 都能看到; lark 之类: 至少是我和对面的人/群, agent 作为我的代理人/观察者)。第一屏实际上是给我看每个'群'的沟通历史的 —— 一个简易 IM, 按来源支持的 feature 不同 (telegram 不支持讨论串只能引用, lark 支持)。"

(In short: an agent subscribing to another agent's messages as an information source is a by-the-way feature — a "message watcher" — and must not be first in the UI. The first thing seen is the GROUP LIST; every group has at least two members (an agent channel: at least two agents plus the owner as a virtual observer who can post into it; Lark and the like: the owner and the other side, an agent as the owner's delegate/observer). The first screen shows each group's history — a simple IM whose features follow what the source supports: Telegram quotes but has no threads, Lark has threads.)

### 22.1 Against what exists (what already is, what is not)

| The picture | Today (before this section) | Gap |
|---|---|---|
| First screen = the group list, by last activity | First screen = a list sectioned by adapter + the tracking pill (a4); a group (conversation) is second-level | Re-order: conversations become first-level, ordered by activity across adapters; adapters/accounts/tracking drop to secondary sections ("Accounts" + "Message watcher") |
| Each group's history = an IM view | channel-window is already a conversation history + composer | Mostly there; the composer's semantics change (22.2) |
| The owner is a member of every group | Lark/Telegram user tokens can already send as the owner (`sendAsUser`), but the composer PROPOSES (P3) | The owner's OWN messages go out directly (IM semantics); propose/policy/approval stays for AGENT drafts only |
| An agent group = ≥2 agents + the owner | the agents adapter treats each agent SESSION as a conversation (exactly the "subscribe to a source" shape) | A definition of "group" (22.3 D1) + a group-level log (convId = the group id, not a session id) |
| Source-dependent features (threads vs quotes) | channel-caps models how messages arrive and platform capability; no reply-model axis | Add an axis `reply: thread \| quote \| none`; the composer draws only what the source supports |
| Agent subscribe/filter/assign = by-the-way | §7's filter/assign is one of the panel's centres | Keep it, demoted to a "Message watcher" section |

### 22.2 Judgement

Reasonable, and SIMPLER than today: it collapses three concepts (adapter, tracking, agent-session-as-conversation) into one — the group (conversation) is the only first-level object; accounts and the watcher hang off it. Storage, delivery and the outbox are already keyed by conversation, so most of this is re-ordering, not rewriting. Two real model changes: ① the owner's own messages go out directly (no propose); ② agent groups need a definition and a group-level log.

One fence first (money): in an agent group every agent message would wake the other members = a turn nobody typed × (N−1); three agents answering each other is an echo chamber. The existing spend authorizer (`peer-message`) caps the count, not the shape. Proposal: an agent's message in a group is by default only APPENDED to the group log and delivered as context on each member's next turn; only an @mention (or a group policy) wakes at once; the owner's (observer's) messages wake by today's rules (that is a user action).

### 22.3 For the owner to decide

- **D1 what an agent group is**: (a) a Task Group with ≥2 sessions is automatically a group (members = its sessions, the owner observing; msg-acl's "within the group" is already that boundary) — recommended; (b) explicit groups.
- **D2 the wake rule for agent messages in a group**: (a) append to the log + deliver on the next turn, wake at once only on an @mention — recommended; (b) every message wakes every member (today's `vibespace-msg` shape, times the member count).
- **D3 order**: after the channel-accounts lane (feat-channel-cred — accounts are a group's source).

### 22.3a The owner's rulings (2026-09-22, same day)

- **D1 = explicit groups; a direct message is a two-member group.** "agent 主动拉群, agent 不再能'私聊', 或者说 agent 私聊本质上就是自动建立一个两人群。每个 Task Group 还是不要自动建群了, 感觉会有 spam 风险。" (Agents create groups themselves; there is no separate DM — an agent's DM is in essence an automatically created two-member group; no automatic group per Task Group, it risks spam.) ⇒ groups exist only by an action: `vibespace-msg` gains a create verb (name + members); today's `send <agent>` stays and now means "find or create the two-member group of these two and post into it" (the same pair is always the same group, idempotent); the owner is an implicit member/observer of every group; who may be invited is still msg-acl reach (within a group / externalVisibility / override, unchanged).
- **D2 = a notify mode per member.** "每个群成员可以自行设置'通知模式', 默认是'下次用户发消息收报告'。" (Each member sets its own notify mode; the default is "get a report when the user next sends a message".) ⇒ each (group, member) has `notify`, set by the member through the CLI, and by the owner for anyone in the panel: `next-turn` (DEFAULT: new group messages are batched into ONE report delivered as context in that member's next USER-initiated turn — zero billed turns, zero echo chamber) / `mention` (woken at once only when @mentioned) / `always` (every message wakes it = billed turns, said in the panel) / `mute`. An @mention is an explicit act: it wakes every mode but `mute` (the owner's @ as observer included); a message without an @ follows the mode. A wake is still a turn nobody typed and goes through the spend authorizer under the unchanged `peer-message` reason.
- **D3 = after the channel-accounts lane (feat-channel-cred).**

### 22.5 Membership operations (the owner, 2026-09-22: "拉群要设计一下, 建群之后是不是还能拉人, 拉人的时候可以附加 context 消息? 我需要手动拉人能力" — design the group invite; can people be added after creation; can an invite carry a context message; I need to add people by hand)

**The group record** (`data/channels/groups.json`, through channel-store's serialized writer): `{id: 'g-<8hex>', name, createdBy, createdAt, pair: [a,b] | null, members: [{member: <conversation id>, joinedAt, invitedBy, notify}], archivedAt}`. Members are keyed by **conversation id** (the claude/codex session id), not the webui session id — a resume keeps the member's identity; the owner is an implicit member, never in `members`. `pair` marks a two-member (direct) group; finding the same pair is idempotent. The group log = an ndjson log in channel-store (append-only, the existing mechanism) with `convId: <group id>`.

**Actions** (who / CLI / panel):

| Action | agent (CLI `vibespace-msg`) | the owner (panel) | Rule |
|---|---|---|---|
| Create | `group create <name> <member…> [--context "…"]` | the group list's "New group": name + members picked from live sessions + an optional opening context | the creator is a member automatically; ≥1 other member; invitees must be within the creator's msg-acl reach |
| Invite | `group invite <group> <member…> [--context "…"] [--quiet]` | group detail "Invite…": pick live sessions + optional context + a "Wake now" switch (default on) | only members invite; the invitee reachable from the inviter; a duplicate invite = a no-op that says so |
| Context | the `--context` text lands in the group log as ONE **system record** `{kind:'invite', by, member, text}` (visible to all) and is the invitee's first content | same | the context is "why you are here"; optional |
| Wake semantics | an invite is an explicit act toward the invitee ⇒ treated like an @: woken at once unless `mute` (a billed turn through the spend authorizer, `peer-message`); `--quiet` joins without waking | the "Wake now" switch | inviting N at once = N wake-ups, the count echoed in the panel/CLI |
| History | the owner sees everything in the panel; the invitee's **report** holds only post-join messages + the context; it may `read <group> --before <ts>` for older history on purpose | — | never the whole history into a new member's next turn (the 10 KiB injection budget) |
| Leave / remove | `group leave <group>`; the creator may `group kick <group> <member>` | group detail "Remove" (any member) | a two-member group minus one = archived |
| Rename / archive | `group rename`; archive from the panel | same | archive keeps the log |
| Notify mode | `group notify <group> <next-turn\|mention\|always\|mute>` (own mode only) | a dropdown per member (the owner may change anyone's) | §22.3a D2 |
| Speak | `send <group\|agent> "…"` (`send <agent>` = find/create the pair's group) | the group window's composer, sent DIRECTLY, never proposed | the owner's messages are signed "You"; @<member> wakes at once |

**SHIPPED (2.369.159) — the table above row by row, as MEASURED (the gate and leg that prove each in parentheses):**

- **Create — SHIPPED.** Agent: `group create <name> <member…> [--context "…"] [--quiet]`; owner: the group list bar's "New group" (name + LIVE sessions grouped by Task Group, each ticked individually — nothing pre-selected, no whole-group checkbox — + an opening context + "Wake now" default on, echoing "will wake 2 agents (2 billed turns)" BEFORE the click). An agent creator joins automatically; the owner is never in `members`; every group holds at least two agents (the owner creating one picks two). Reach = msg-acl `messageable` judged from the creator; one unreachable invitee refuses the WHOLE call — nothing written, nobody woken, the uniform "not found or not reachable" (no existence oracle); a session the user OPENED (the reachability override) is invitable across Task Groups (widen-only). A name several sessions share ⇒ `ambiguous` WITH the candidate conversation ids, never guessed. A Background Work job (jbt_) creating ⇒ `job-token`, refused locally by the CLI (zero requests) and again by the route (403). (test-channel-groups §1/§3, test-msg-cli-groups §1/§2, test-channels-groups-e2e: each of the two stub CLIs recorded EXACTLY ONE invite-wake frame)
- **Invite — SHIPPED.** `group invite <group> <member…> [--context] [--quiet]`; the panel's "Invite…" = the New group dialog without the name. Members and the owner invite; a non-member's invite is answered word for word like a group that does not exist; a duplicate = a no-op named (`already-member`), nobody woken; **a two-member (direct) group takes no third member** — `pair-group` 409, "create a group to add people" (not in the table above; decided in the build: the pair group is `send <agent>`'s idempotent target, and a third member would leave the pair without its direct conversation); at most 32 members. (test-channel-groups §1/§3)
- **Context — SHIPPED.** One system record per invitee `{kind:'invite', by, member, context, wake}` in the group log (visible to all; the panel shows the context under the record); it is the invitee's first content: the invite wake's report leads with "You were added by <who> — context: …" (the context takes at most a third of the report budget). Context ≤ 4000 characters, frame-inert (agent-controlled text). (test-channel-groups §1c "the invite CONTEXT comes first"; e2e "the invite frame carries the lead line AND the opening context")
- **Wake semantics — SHIPPED.** An invite is an explicit act toward the invitee: `wakeVerdict` answers wake for the invitee's OWN invite record (every mode but `mute`), `--quiet` ⇒ `quiet-invite`, no wake; inviting N = N authorizations (the recording authorizer asked N times, reason `peer-message`), the CLI echoing "woke 2 invitees = 2 billed turns: …", said at 0 too; a refused wake is marked `(not billed)` with the spend ceiling's reason. A wake = that member's report delivered NOW down THE ladder (`deliverToConversation`, `spendReason:'peer-message'`), the authorizer deciding; a refused wake is audited (`group-wake`) and NOT stashed (the message is in the log and rides the next report; a stash would deliver it twice). A member woken by its invite is not handed the same content again on its next turn. (test-channel-groups §1b — the whole table, 4 modes × 8 message kinds — and §3; test-spend-paths' ladder allowlist)
- **History — SHIPPED.** The owner sees the whole log in the panel. A member's report (`reportFor`) holds only records after its JOIN and after its `reportedUpTo`, never its own; the newest are kept, older ones clipped with a `vibespace-msg read <group> --before <oldest shown ts>` pointer (`upTo` still covers the clipped ones, so they are never re-reported); the newest always shows (clipped to the room if it must). Budgets: one group's report 2 KiB (the model's default), a wake's report 4 KiB (it rides its own turn, not the 10 KiB injection), and **all group reports on ONE user turn together ≤ 4 KiB**, LAST in the prompt-context delivery and budgeted from what the 9600 B hard cap left; a group with < 320 B of room waits for the next turn rather than arrive as a stub. `read <group> [--before ts] [--limit n]` reads history on purpose; a non-member reads nothing (uniform not-found); a member may read an archived group. (test-channel-groups §1c/§3c/§4 "the whole delivery stays under the 9600 B inline cap")
- **Leave / remove — SHIPPED.** `group leave <group>`; `group kick <group> <member>` by the creator or the owner only, kicking yourself ⇒ `self` ("use leave"); the panel's group detail has "Remove" per member (the owner may remove anyone). **A two-member group minus one = archived** (the log kept; the same pair's next `send` opens a NEW direct group); a three-member group losing one stays live; a group with no agent left is archived too. (test-channel-groups §1/§3c)
- **Rename / archive — SHIPPED, with one CLI verb more than the table.** `group rename` by any member or the owner; archive also has a CLI verb `group archive` (the creator or the owner; archiving twice = a no-op). Archive keeps the log; an archived group takes no invite or rename and folds apart in the group list. (test-channel-groups §1; test-channels-groups-ui "an archived group is listed apart")
- **Notify mode — SHIPPED.** `group notify <group> <next-turn|mention|always|mute>` changes only the caller's own (the request carries no member field at all); the panel's group detail has a select per member, the owner may change anyone's (e2e: the owner set alpha → always, beta untouched); a jbt_ notify ⇒ `job-token`. The four modes measured: next-turn = zero authorizations and ONE report on the next USER-initiated turn, exactly once; mention = an unnamed message wakes nobody, an @ wakes only the named; always = the authorizer asked for every message (a refused one still logged and still reported); mute = not even an @ wakes it, and no report. "User-initiated" = the session's `_userInputAt` (ws input / chat-input) is not older than its `_machineInputAt` (every ladder rung + auto-resume's continue) — a machine-started turn gets no report, so the echo chamber is impossible by construction. (test-channel-groups §1b/§3b/§4 through the REAL prompt-context route)
- **Speak — SHIPPED.** Agent: `send <group|agent> "…" [--wake]` — `send <agent>` = find or create the pair's two-member group (`pairKey` symmetric: both directions land in the SAME group, exactly one exists), next-turn by default = zero billed turns, `--wake` = an @ of the other member = one billed turn; the answer echoes the wake count. Owner: the group window's composer sends DIRECTLY as "You" (never proposed — the group window has no proposal path at all), `@` autocompletes member names, a preview under the box names who THIS text would wake (parity-tested against the engine's `wakeVerdict` over 320 rows; the control: a preview letting an @ beat mute is caught by the same comparison), and the toast after the send says how many the server actually woke. On an external conversation (Lark/Gmail) the owner's own message also goes out directly where `sendAsUser` is offered (`POST /api/channels/:a/:c/send`, `direct:true`: no policy, no guard, still the outbox record / audit / never-re-sent `unknown`); an agent's `direct` is ignored and agent drafts keep the outbox cards. (test-channels-groups-ui §1/§2; e2e: the `always` member's stub recorded the wake frame, the `next-turn` member's stub recorded nothing) **r1:** every wake an agent's act would cause (@, `--wake`, invite, `always`) is paced — one per (sender, receiving conversation) per 30 s, keyed by the resolved conversation; a floored wake is a refusal (not billed, the message still posted); a bare name that is both a group and a session is refused `ambiguous`; a sender with no conversation id yet is refused `bad-member` (test-msg-cli-groups §2).
- **r2 (the second adversarial round, nine findings, each reproduced before its fix) — SHIPPED.** ① **One command cannot spend a slot's hour**: beside one wake per (sender, receiver) per 30 s, **at most 8 wakes per sender per minute** (before: one `group create` of 25 = 25 billed turns, 62 in 31 s from one sender); and **the count comes BEFORE the act**: an agent's act that would wake **more than 5** is refused `confirm-wakes` (with `wakes: N`, nothing written) until it says `--yes`. ② **The owner's routes**: the panel sends the count its preview said (`expectWakes`), the engine counts the wakes the act would cause inside its door, and any other number (a missing echo = 0) is `wake-count-mismatch`, nothing written — a local script cannot wake members signed "User" without first knowing and saying the cost; with **auth off** those routes are open to any local process, so the owner's post/create/invite are then paced exactly like an agent's (with auth on a cookie proved the owner, unpaced). ③ A corrupt `groups.json` (and index/adapters/outbox) is no longer read as empty and overwritten: it is renamed `<name>.corrupt-<ts>` with its bytes intact (never unlinked), one log line, one "For you" item, and only then does the store start empty; if the rename fails that store refuses every write. ④ A long message cut in a report is followed by `(N message(s) above cut short — the whole text: vibespace-msg read <group> --before <ts> --limit <n>)`, which returns the whole text. ⑤ Unread counts are no longer re-read from every unread group's whole log on every broadcast (~100 ms of main loop per agent post at 200 groups × 500 messages): memoised per (mark, lastAt), moved by the one log-append door, recomputed only when a mark moves — ≈ 1–2 ms per post. ⑥ The pace ledger is persisted (`data/channels/wake-pace.json`) — a restart forgets no floor. ⑦ A turn with no room for any report still NAMES the waiting groups (it was silence). ⑧ A wake the authorizer refused (not billed) refunds its pace slot instead of flooring the next legitimate wake with words that said a turn was billed. ⑨ Terminal mouse reports (wheel / click — the CLI turns mouse tracking on) are not a person typing and no longer turn a woken, billed turn into a user turn. (test-channel-groups §1c/§2b/§3e–§3h/§4c, test-msg-cli-groups §1/§2, test-channels-groups-e2e)
- **r3 (the third adversarial round, eight findings, each reproduced before its fix) — SHIPPED.** ① **Three side doors beside the fenced group routes**: with auth off, the Channels composer's `/send` and `/propose` and the outbox `/approve` woke any live session through the built-in Agents adapter with no echo and no pace (ten cookie-less sends in a second = ten billed turns; a target the group route had just floored was woken again). The adapter MODULE now declares `sendStartsTurn` (the engine reads the declaration, never the adapter id): a send that starts a turn and goes out now passes the echo (`expectWakes`, else 409 `wake-count-mismatch`, nothing created) and — auth off — THE SAME persisted pace as the group routes (`user|<conversation>`, else 429 `rate-floor`, nothing created, an approve's proposal stays awaiting); a send that did not go out refunds its slot. The composer says "it wakes this agent: 1 billed turn" before the click, the approval card "Approving wakes this agent"; an agent's `vibespace-channels reply` into the Agents adapter is paced by its own pacer; test-architecture §49 censuses every owner route in routes/channels.js that can start a turn. ② **A renamed member made every owner Send a 409**: the panel counted @mentions against the names it drew, the server against live names, and a rename broadcast nothing. The 409 now carries the view the server counted against (the composer repaints before its next click), and the session list's one notify point calls `noteRoster()`, which announces exactly the groups whose member names / liveness moved. ③ The heavy e2e runs with auth ON (cookie login) and no longer sleeps out a 30 s floor (43 s → 14 s). ④ A stamp in `wake-pace.json` more than 5 s in the future is not a wake (a backward clock step no longer floors every pair for the step's length); dropping them is logged. ⑤ A refused wake refunds only its (sender, receiver) stamp — the attempt still counts in the sender's 8 per minute, so under a spend refusal at most 8 attempts a minute reach the authorizer (was 60 in 6 s). ⑥ A BLOCKED `index.json` refuses `update()` too (an action used to succeed on screen and vanish at restart); every blocked refusal carries `code:'store-blocked'` (503) and every refused write is logged. ⑦ A set-aside `adapters.json` is on the first screen: the digest carries `quarantined` and the panel draws a warning line above the group list. ⑧ A blocked file's inbox headline says it could NOT be set aside (it said "set aside as" its own name). (test-channel-groups §1e/§2c/§3i–§3k, test-channel-store §r3, test-architecture §49, test-channels-groups-e2e)
- **r4 (the fourth adversarial round) — SHIPPED.** A throw between the wake grant and the send (a blocked outbox's 503, an EACCES/ENOSPC write) kept the pace stamps: the retry once the store was back was a 429 whose words blamed a wake that never happened, and eight such errors floored an unrelated group wake for a minute. `propose`/`approve` now give the slot back on a throw — the pair AND the sender's minute when nothing reached the authorizer or the adapter; a throw after the request may have left keeps the floor (that wake is real).

**The first screen — SHIPPED.** The group list = every agent group + every TRACKED external conversation of the connected accounts, ONE list by last activity (a row = source glyph · name · time / source chip · last line · unread), repainted IN PLACE from the broadcasts (e2e: zero `/api/channels*` or `/api/channel-groups*` fetches); unread = the records after the owner's read mark that the owner did not write (only the owner's own act moves the mark), and the rail's Channels badge counts the groups' unread too; "Accounts" and the "Message watcher" are secondary sections below, their fold kept in user state and followed across clients.

**Not shipped (still design):** 22.1's source reply-model axis (`reply: thread | quote | none`, the composer drawing only what the source supports) was NOT built in this round — channel-caps has no such axis yet; the composer splits by `sendAsUser` / bot / read-only.

**Implementation notes.** PURE `src/channel-groups.js` (the model: record, validate, symmetric `pairKey`, membership as pure transitions from a closed error set, `mentionsIn`, `wakeVerdict`, `reportFor`) + ORCH `src/server/groups-engine.js`. The group log is filed under `adapterId: 'groups'` — NOT `'agents'` as the record paragraph above first said: the agents adapter's conversations are SESSIONS, a group is another kind of object, and sharing one adapterId would collide the two convId spaces. Member rows also carry a `name` snapshot (so a report can sign a member whose session is gone) and `reportedUpTo`; the group record carries `lastAt`/`lastText` (the list's activity order; recomputable from the log) and the file carries the owner's read mark `ownerRead` beside `groups`. The agent CLI (g2) and the panel (g3) are described above; the owner side is `/api/channel-groups` (list / messages / roster / create / post / invite / kick / rename / archive / notify / read).

**Panel:** the group detail = the member list (name + notify dropdown + Remove) + an "Invite…" button + the log. The invite dialog: live sessions multi-select (shown grouped by Task Group, but never added as a whole group — D1), a context box, a "Wake now" switch, and the echo "will wake N agents (N billed turns)".

**Not in scope:** sub-threads inside a group, group avatars/announcements, cross-instance groups (remote agents do not join groups yet).
