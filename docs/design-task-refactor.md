...
**Update 2.121.0 (2026-07-13): the group-level Checklist was REMOVED entirely (user decision — agents don't care about other agents' backlogs; work items live at the session level as the agent's own todo/Steps). Every Checklist/`plan-*` mention below is historical; stored `plan` arrays stay dormant in the JSON. 2.122.0 then added a DIFFERENT group-level list: the **Backlog** — a parking lot for non-immediate items (deferred decisions / future work), summary-only injection, seeded once from the dormant plan's unchecked items.**
