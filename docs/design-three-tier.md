...
| R3 ◐ step 1 = 2.283.0 | Transcript service | extract the parse stack behind one interface (pure refactor — DONE: src/transcript-service.js, HTTP read family rerouted) → daemon-hosted ops dark behind a flag → byte-identical parity → remote reads switch. Replacement substrate (mirror + location index) proven BEFORE the raw remote cache demotes to fallback | the slab-sync integrity class; faster remote history |
