...
| R4 ◐ step 1 = 2.286.0 | Usage + events | walker into the daemon (DONE: `src/usage-walker.js` bundled, `usage-scan` op in a child process, two-phase cursor commit closes the relayed-transfer loss window, per-op capability gating via hello `capabilities`; script-ship and ssh remain the fallback ladder) → `session.events`; policy consumers subscribe; local double-feeds during cutover (id dedup makes overlap harmless) | remote live-estimate + remote codex coverage gaps; the shipped scanner and its parity test |
