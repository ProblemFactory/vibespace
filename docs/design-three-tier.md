...
| R5 ◐ step 1 = 2.288.0 | Discovery | snapshot walk moved into a daemon child process (loop never blocks on a slow home fs; inline fallback pinned byte-identical) → on-device claims (`discovery.v2` op, dark + parity like R3/R4) → local discovery harvests device #0 behind a flag once poll latency is proven | the ssh discovery script (to fallback) |
