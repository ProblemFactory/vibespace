...
| Ctx sync | **DIVERGENT** | local = no sync (direct dir); ssh = rsync, no caps; dial = per-file hash with ≤400 files / ≤2MB caps ⇒ a 3MB context file silently never reaches a dial device. |
