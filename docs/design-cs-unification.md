...
| Machine snapshot (sysinfo) | **UNIFIED** (2.314.0) | Was a MISSED twin (added 2.226.3, predating the metric): local = `src/sysinfo.js` in-process, remote = a separate ssh script — and the interpretations of "used memory" DRIFTED (the false-100% incident: raw `memory.current` vs working set; the remote script had been right all along). Now the daemon bundles the module and serves a capability-gated `sysinfo` op; the ssh script is the fallback rung for daemon-less hosts only. Found by the user, post-closure — evidence the metric needs a standing sweep, not a one-time audit. Test: scripts/test-sysinfo-op.mjs. |
