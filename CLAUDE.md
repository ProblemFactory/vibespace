...
- Claude fork behaved identically to resume in chat: the WebUI **never captured Claude's stream-json `session_id`** (zero refs anywhere — for new sessions the id was filled in lazily by lock-first discovery). `--fork-session` makes claude mint a NEW session id at startup and write a separate JSONL (verified empirically: plain resume reuses the id, fork returns a fresh one), but the server kept tracking the PARENT id passed at creation — so a forked window shadowed the original (same name/history/resume target) and the fork's transcript was orphaned. Fix (mirrors the Codex thread-id adoption at the `nextThreadId` block): the stdout parser adopts `msg.session_id` for Claude when it differs from the tracked id, records the parent in `forkedFrom` (informational for Claude — discovery never hides on it, unlike Codex), and persists `claudeSessionId` in session meta. Guarded by a one-shot `session._forkRequested` flag (set in ws-handler only when `data.fork` + Claude) so a normal resume — whose id the parser also sees on every line — can never be hijacked. The client already propagates the adopted id to the window `_openSpec` via `syncSessionIdentity` on the next `active-sessions` broadcast (the same path Codex fork/resume uses). Needs a server restart to take effect (dtach sessions survive it).
