...
- **Release discipline**: every change = version bump + CHANGELOG + `npm run build` (arch suite runs inside it) + commit + push; daemon changes also `npm run build:agentd`; user self-updates — no agent-initiated restarts.
