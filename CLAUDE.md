...
src/
  ...
  server/auto-resume.js — 撞限后自动续跑(2.368.0): CLI那套只在TUI(stream-json没有/rate-limit-options), 这是我们自己的且**跨重启存活**(CLI自己重启即取消); 顺序=先让account pool换号(秒级)换不动才arm; 三态门(会话值>claude.autoResumeOnLimit默认关), 不早发/不重复/不在streaming时发/恢复即解除, 续跑用CLI原话. 同版还有输出风格(Concise等)=spawn时经唯一的--settings设, stream-json不能中途切. ⇒ kb-file-structure.md
