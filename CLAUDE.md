...
src/
  ...
  server/conversation-deliver.js — THE conversation delivery ladder (2.362.0): channel socket→local inbox→owning machine's agentd peer-post op→durable stash drained at injection; jobs通知与agent消息同一实现. ⇒ kb-file-structure.md
...
- PEER MESSAGE INVISIBLE ON LIVE STREAM (2.362.2, inc-mt27t0bg userW): CLI收站内信时stdout从不带user记录(仅JSONL有), 唯一stdout载体是result.origin — _processResult挖掘+三站点msg_id去重; 不变量=stdout流与JSONL是不同记录集, "刷新后可见"≠"live可见"两者都要测…
