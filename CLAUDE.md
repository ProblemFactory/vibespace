...
src/
  ...
  server/published-pages.js — 发布页(2.364.0 接管artifact; 2.366.0 publishContent上传路径+srcKey=host:path+会话归属+onPublished唯一通知点+浏览器origin启发): /p/<id>自托管可分享HTML, 默认私有+public锁, srcPath upsert稳定URL, CSP sandbox=同源XSS防线(勿加allow-same-origin). ⇒ kb-file-structure.md
