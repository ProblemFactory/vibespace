...
- Blank chat window on huge sessions (>512MB JSONL): `readFileSync(fp,'utf-8')` THROWS past Node's ~512MB max string length; the swallowed error left attach with empty history. Fix: `readJsonlBounded` (codex adapter, shared with Claude session-store) — files >34MB load a line-aligned 2MB head + 32MB tail, middle elided with a visible `<system-reminder>` marker message at the seam (timestamp copied from neighbor so codex timestamp-sort keeps it in place; shows ~record count + MB omitted). Also prevents hundred-MB synchronous parses from blocking the event loop. The elided middle IS reachable on demand: `getJsonlLineIndex` builds a cached line→byte-offset table (streaming Buffer scan, no >512MB string), `jsonlGapInfo` reports the head/tail line boundaries, and `readJsonlLineRange` seek-reads any raw line range. `GET /api/session-history-gap?…&info=1` returns the gap bounds; `…&endLine=N&count=C` seek-reads + normalizes a slab standalone. The seam marker renders an interactive 'Load earlier messages' card (chat-renderers `_renderHistoryGapMarker`); clicking walks one ~2000-record slab older each time, filling the gap bottom-up. Gap messages carry `.chat-gap-msg`, are excluded from virtual-scroll trim + `_windowStart/_windowEnd` accounting, and render read-only (tool calls whose result is outside the slab show as orphans — acceptable for history browsing). Search covers the WHOLE file: `searchJsonlFull` (codex adapter) streams the entire JSONL (raw-substring pre-filter → parse candidates → match extracted text; 500-match cap + truncated flag; 222MB ≈ 0.5s) via `/api/session-history-gap?search=q`, returning `{line, ts, preview}` matches; ChatSearch auto-switches to this full-file mode when the gap minimap is active and jumps via `ChatView.jumpToFileMatch` (middle: `_seekToGapLine` slab load with an already-loaded fast path via `_gapElForLine`; head/tail: `_jumpToFileTime` + nearest-ts), expanding the target card so highlights paint. The MINIMAP, however, covers the whole conversation: `scanJsonlUserTurns` (codex adapter, mtime-cached) streams the file once (substring pre-filter `"role":"user"` → parse only ~2k candidate lines, ~1.2s for 541MB) and `/api/session-history-gap?fullturnmap=1` returns every user turn in TIME coordinates (the shared axis between loaded messages and the elided middle) plus each turn's file line. ChatMinimap's full-extent mode (`renderFullExtent`) positions markers by `(ts-firstTs)/(lastTs-firstTs)`; the thumb tracks the visible viewport's ts span (`setVisibleTsRange`, fed by `_reportVisibleTsRange`). Clicking/dragging a marker calls `_jumpToFileTime(ts, line)`: a middle (unloaded) line seek-loads a forward slab there (`startLine` param) and scrolls to it; a head/tail line scrolls to the nearest rendered message by ts (jumpToIndex(0)/jumpToBottom fallback if not rendered). Messages carry `srcLine` (threaded from each record's `__line` through both normalizers) and DOM `data-ts`/`data-line` for positioning.
