...
- **Release discipline (TWO-TIER GATE since 2026-09-07, B-4c5a — the single 11.5-min battery was a gate people routed around)**: every change = version bump + CHANGELOG + commit + push. Since 2.369.104 every suite run and every heavy launch REAPS scratch-dir orphans (ci.mjs `scratchOrphans`: /tmp/vs-* root + dir gone or unowned-and-stale; `node scripts/ci.mjs --reap` by hand) — 504 leaked device daemons once took the box down. … ⇒ kb-design-lessons §18
