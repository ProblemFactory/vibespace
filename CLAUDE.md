...
- `GET /api/workflow?runId=&claudeSessionId=&cwd=` — parse a dynamic-workflow (ultracode) run's TERMINAL snapshot `<projDir>/<claudeSessionId>/workflows/wf_<runId>.json` → normalized `{workflowName, status, agentCount, totalTokens, totalToolCalls, durationMs, phases:[{title, agents:[{label,model,state,agentId}]}], result|error}`. runId-validated; targeted-then-session-scan finder (runId is globally unique). 404 until the run finishes (snapshot is written ONCE at terminal state, never live)
