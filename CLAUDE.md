...
| **Pool/billing decisions** | src/account-pool-auto.js (pure) + the engine in server.js; per-session links (plan C) resolve via poolCurrentFor(pool, webuiId). Every blocked outcome must SPEAK with named buckets. … **EVERY reading AND every rejection is attributed to the credential SLOT (the token-slot-validated link), never to the OTel-observed org** — the slot is resolved ONCE PER TURN (`rejectionSlotFor` / `readingSlotFor`), every blocked outcome SPEAKS with named buckets, placement follows the REQUEST model (lock > pick > spawn > served-if-not-a-fallback); USAGE CREDITS (B-ad05): `overageState().mode==='allowed'` ranks a member LAST (last resort only), a parked pool answers `on-credits` + ONE 6 h notice, spend-guard unchanged. ROOT FIX DESIGNED, NOT SHIPPED: docs/design-account-hardening.md (ONE lease + ONE write choke point + ONE spend authorizer). ⇒ kb-design-lessons §18 | test-pool-auto (110) + test-account-pool (33) + test-account-verdicts (45) + test-auto-resume (132) + test-auto-resume-loop (191) + test-login-expiry (257) + test-readings-attribution (393) … |
| **A channel ADAPTER, or anything about a channel RECORD** (docs/design-communication-panel.zh.md) | The adapter goes in `src/channels/<kind>.js` behind the registry's contract (`src/channels/index.js`) and NOTHING downstream learns its name — **gate on the capability row, never on an adapter id**, exactly as `src/backend-caps.js` does for harnesses. … ⇒ kb-design-lessons §18 | test-channel-record (incl. the grep-derived FRAME CENSUS over every hyphenated tag the tree writes) + test-channel-caps (incl. r3's `off` leg with a scratch-dir patched copy) … |
...
| **A desktop application window / a display backend** (docs/design-desktop-apps.zh.md, P8-1 2026-09-13) | PURE `src/desktop-apps.js` (registry rows + validation, the `DISPLAY_BACKENDS` capability table — one row per rung `{perWindow, adaptive, needs, stream, wired}` — the `resolveBackend` ladder that judges EVERY rung and reports each one's reason in §3's exact words … ⇒ kb-design-lessons §18 | test-desktop-apps (123, §7 = every noVNC encoder parsed out of node_modules and driven through the sieve) + test-desktop-display (72, real Xvfb/x11vnc, SKIP with reason) + test-vnc-view (49 … |
...
src/
  ...
  usage-walker.js — THE ledger walk, ONE module (2.297.0 twin-killer): UsageHistory.scan() consumes it IN-PROCESS (cursors injected, onEvent enrichment orchestrator-side), the daemon runs it as the usage-scan op child … ⇒ kb-file-structure.md
  ...
  channel-store.js — SHARED (fs+path only) persistence primitives for data/channels/ (design §5): adapters.json · index.json · msgs/<adapterId>/<convId>.ndjson (APPEND-ONLY) · audit.ndjson rolled by DATE into archive/ (the day comes from the file's OWN last record, never mtime … ⇒ kb-file-structure.md
  ...
  channels/lark.js — ORCH (design §6.3/§12.1/§13, P1a) THE LARK / 飞书 READ adapter behind the §4 contract: a USER token minted by the loopback's FIXED mode (the registry's callback URL, imported never spelled), `im/v1/chats` discovery (paged) … ⇒ kb-file-structure.md
  channels/gmail.js — ORCH (design §6.3/§12.2/§14.2, P1a) THE GMAIL READ adapter — the DELEGATING row: the OAuth client comes from `resolveIntegration('gmail')` which resolves through the existing `VIBESPACE_GDRIVE_CLIENTS` presets (decision 5: `prefer:'channels'` … ⇒ kb-file-structure.md
  ...
  session-status.js    — SessionStatusManager (session-level state/urgency set by the AGENT via vibespace-status CLI or by the user; user overrides of agent-set values record a pendingNotice injected as <system-reminder> into the NEXT chat-input … ⇒ kb-file-structure.md
  ...
  desktop-apps.js — PURE (imports only keeper-limits) the desktop-app MODEL (docs/design-desktop-apps §2 row 1): registry rows + `validateAppRow`/`validateLaunchRequest` (an exec is a HUMAN's — registry or dialog — never an agent's, §5), `DISPLAY_BACKENDS` (xpra → vnc-display → desktop-singleton … ⇒ kb-file-structure.md
  ...
  server/desktop-stream.js — ORCH THE ONE ws bridge (§2 row 4): `upgradeId(pathname)` (`/api/vnc` = DESKTOP_SINGLETON_ID, `/api/desktop/<id>/stream` = id) + `handleUpgrade(req, socket, head, id)` — cookie auth INSIDE (401), `resolveTarget(id)` → 404 when nothing serves it … ⇒ kb-file-structure.md
  routes/desktop-apps.js — GET/POST /api/desktop/apps, GET /api/desktop/apps/:id, POST …/:id/stop, POST …/:id/keep-alive (+ the singleton's GET /api/vnc/status · POST /api/vnc/start, moved here from server.js unchanged); every route takes `host` and REFUSES a non-local one by name (400 unsupported-host); every failure answers `{error, code}` with a status by code. ⇒ kb-file-structure.md
  ...
  **CS SEPARATION RULE (2.276.0): `hostId` is a PARAMETER, never a branch.** Server-side logic is written ONCE against a machine handle; the only difference between this machine, an ssh host and a dialed-in device is the transport underneath it. `hosts.device(falsy)` returns device #0 (the local daemon has been a full DeviceManager since 2.158.0 — same binary/mux/op surface — but was unreachable through the API, so every feature was written twice BY CONSTRUCTION). Migration table: docs/design-cs-unification.md — the metric is PARALLEL IMPLEMENTATIONS (9 twin-sets → 0 unjustified as of 2.279.1) … ⇒ kb-file-structure.md
  ...
  lib/
    ...
    manage-agents.js — installManageAgents(App): Manage-Agents dialog + Anthropic/ChatGPT account rosters/wizard (~790 lines, split from app.js). **2026-09-14: every roster donut is a column with its OWN compact reset countdown under it (src/lib/usage-eta.js** … 2026-09-15: each row ALSO carries the account's next reset in full (`2d21h38m`, `accountResetEta`) and the two soonest rows of a list are highlighted (`markSoonRows` → `.usage-acct-soon`, a 30 s re-spell tick); 2026-09-16: a label-less donut column keeps a 10 px slot and the next-usable cell is a FIXED 12.5ch column (icon left, number right) so nothing wobbles row to row. ⇒ kb-file-structure.md
    ...
    window.js          — WindowManager (drag/resize/snap/grid; 2.369.112 `syncHiddenViews()` = ONE derivation of the mobile/tab/minimized hiders → every chat view's `setHidden` reasons, inc-mu6bfv1t-4drq) ⇒ kb-file-structure.md
    ...
    contributions.js   — COMMANDS + MENUS (when/group/order) + KEYBINDINGS REGISTRY (Plugin Ph1): registerCommand/registerMenuItem/menuItems(menu, ctx)/registerKeybinding + ONE document dispatcher/runCommand; DOM-free at import; explicit separators; loud unknowns (throw / warn-once + telemetry). … ⇒ kb-file-structure.md
    ...
    sidebar.js — Sidebar shell (filter/sort/merge pipeline, tab switching). **`LIVE_SESSION_FACTS` + `liveSessionFacts()` (2026-09-07 r3) = THE list of per-session live facts the `active-sessions` payload publishes under the names every client surface reads them by.** `_merge()` REBUILDS the row … ⇒ kb-file-structure.md
    ...
    channels-panel.js + channel-window.js — CLIENT: the Channels rail panel (adapters as sections, **a FRESHNESS CHIP on every row** — the one number a user needs before handing something to a lane, drawn from the lane ACTUALLY carrying it — the `channel-row` menu and the ⚙ row as CONTRIBUTIONS registered by the owning … ⇒ kb-file-structure.md
    channel-outbox.js + channel-reach-editor.js — CLIENT (design §8 / §9.2 / §9.5, P3): ONE approval-card renderer for the inline section of a conversation window AND the singleton Outbox window (`channel-outbox` type, ⚙ row, panel button carrying the awaiting count) … ⇒ kb-file-structure.md
  ...
  sidebar-render-mobile.js — Mobile sidebar mixin (two-level folder/group navigation)
    ...
    chat-status-bar.js — ChatStatusBar (model, context%, cost, permission mode, task popup; B3 `setTurnState` = harness 自报的回合态, 只为 `requires_action` 画芯片——idle/running 由 composer 的 spinner 说, 一个事实不许两个声音; null≠idle=没上报过就什么都不画; 会话死亡由 chat-view 的 `_retireLiveClaims()` … ⇒ kb-file-structure.md
    ...
    desktop-app-window.js — window type `desktop-app` (§2 row 7): one window per app session (`_desktopAppId`), openSpec `{action:'openDesktopApp', id}` replayed by layout sync on every client (N viewers on one `-shared` display), title = the label via setTitle (textContent) … ⇒ kb-file-structure.md
...
- | **Desktop apps / a display backend** | `src/desktop-apps.js` (PURE ladder + model) + `src/desktop-display.js` (SHARED facts) + `src/server/desktop-app-keeper.js` + `src/server/desktop-stream.js` + `src/routes/desktop-apps.js` + `src/lib/vnc-view.js` / `desktop-app-window.js` / … ⇒ kb-bugfix-invariants.md
