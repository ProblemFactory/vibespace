...
- Blank chat window on huge sessions (>512MB JSONL): `readFileSync(fp,'utf-8')` THROWS past Node's ~512MB max string length; the swallowed error left attach with empty history. Fix: `readJsonlBounded` (codex adapter, shared with Claude session-store) — files >34MB load a line-aligned 2MB head + 32MB tail, middle elided with a visible `<system-reminder>` marker message at the seam (timestamp copied from neighbor so codex timestamp-sort keeps it in place; shows ~record count + MB omitted). Also prevents hundred-MB synchronous parses from blocking the event loop. Middle records are NOT scrollable — search/minimap/pagination operate on the loaded window only.
