...
- **Status bar indicator**: 🎯 + status icon (▶ active / ⏸ paused / ✓ complete) + elapsed (from `timeUsedSeconds`, not wall clock — updates per turn) + truncated objective. Click → popup with full text, "Pursued for Xm · status", Continue button (only when not active), Clear button.
