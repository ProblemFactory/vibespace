...
src/
  ...
  server/auto-resume.js — **2.369.104 the delayed ARM notice is worded by the arm's CAUSE (`armNoticeFor`: a hot switch names the member + seconds; only a real reset says 重置).** **2.369.97 ONE CARD PER CONTINUE: the cause rides the delivered prompt (`sendToSession(id, s, text, {note})` ⇒ `originNote` / `webui_origin_note` ⇒ card head "VibeSpace auto-resume — <cause>")** … ⇒ kb-file-structure.md
