...
- Session card layout is TWO ROWS (2.37.1, `.session-card-lines` wraps `.session-card-main` + `.session-card-sub`; left control icons center across both via the row's `align-items:center`): row 1 = name (flex) + connection badge + expand arrow; row 2 = role/config/host/**status chip**, and `.session-card-sub` hides itself (JS, after the chips are set) when it has no visible child so plain sessions stay one line. The per-session config badge (`.badge-config`) is icon-ONLY (gear) with model/effort/permission in its `title` (printing the full model id inline crowded the row). Task board headers are likewise two rows (2.37.1, `.folder-header.has-paths` → `column`; `.folder-header-main` = title + tags + count, `.task-folder-paths` = the task's linked folder paths, recursive shown as `/**`) — the `[data-colored]` title-color selector is a DESCENDANT match now (folder-path moved into `.folder-header-main`).
