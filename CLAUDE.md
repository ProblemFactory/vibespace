...
- **Release discipline**: every change = version bump + CHANGELOG + commit + push; `git push` runs the MANDATORY release gate (`npm run ci` — build + gate suites + headless-chrome CLIENT boot smoke + worktree server boot smoke w/ route battery, ~1min, tracked pre-push hook + GitHub Actions mirror; docs-only pushes skip; `VIBESPACE_SKIP_CI=1` = emergency bypass, use never). Daemon changes also `npm run build:agentd`; user self-updates — no agent-initiated restarts.
