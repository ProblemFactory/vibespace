...
- **Release discipline (TWO-TIER GATE since 2026-09-07, B-4c5a — the single 11.5-min battery was a gate people routed around)**: every change = version bump + CHANGELOG + commit + push. … ⇒ kb-design-lessons §18
...
scripts/ci.mjs — THE RELEASE GATE, TWO TIERS (B-4c5a, 2026-09-07): the tier table `{name, tier:'fast'|'heavy', why}` + EXCLUDED-with-a-reason, both runners, the detached `--heavy-launch`, the push-blocking `--check-heavy`, `--status`, `--census`, and (round 2) … ⇒ kb-file-structure.md
...
src/
  ...
  server/auto-resume.js — **2.369.97 ONE CARD PER CONTINUE: the cause rides the delivered prompt (`sendToSession(id, s, text, {note})` ⇒ `originNote` / `webui_origin_note` ⇒ card head "VibeSpace auto-resume — <cause>")** … ⇒ kb-file-structure.md
