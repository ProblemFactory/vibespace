...
- Broken pty stdin false positive: auto-detect killed working pty when API was slow (meta.streaming still false after 5s). Fix: check buffer length growth as primary signal — if ANY output arrived, stdin works.
