...
- **Claude**: wrapper stores `meta.goal`; after each `result` (turn end, non-error) it auto-sends a continuation user message (`[Goal still active: ...] Continue...`) so the model keeps working. CLI's own `/goal` Stop hook is also surfaced — stream-json emits `goal_status` attachments (`{condition, met, sentinel}`), detected in the PTY handler and by `MessageManager._processAttachment` (→ `goalState()`).
...
- `/goal` not working in stream-json mode: slash commands are terminal-only; in stream-json `/goal` is echoed as plain text. Fix: implement at wrapper level — Claude auto-continues after `result`; Codex uses native `thread/goal/set` RPC. CLI's own goal is surfaced via `goal_status` attachments (Claude) / `thread/goal/get` (Codex).
