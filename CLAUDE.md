...
data/bin/vibespace-hook.mjs — Dual-harness SessionStart hook (task context injection; generated at startup; env-gated no-op without VIBESPACE_TASK_ID; registered idempotently in ~/.claude/settings.json + ~/.codex/hooks.json by ensureAgentHooks — only the vibespace entry is ever touched)
...
- `GET /api/agent/task-context?taskId=` — SessionStart hook payload (Bearer `vsst_`): rendered task state + contextDir file index + rules
...
- vibespace-task CLI (2.33.0, P3 local): agents report task-level updates — `progress "note"`, `plan-check <n|text>`, `plan-add`, `status <enum>`, `show` — scoped server-side to the session's context task; taught by the injected context rules. Remote sessions NOT yet covered (needs the ssh reverse tunnel + remote tool distribution — remaining P3).
- Task context injection (2.32.0, P2): a session created/resumed with a task gets the task context (state + objective + plan + progress + context-folder file INDEX + rules incl. vibespace-status usage) injected at start. CLAUDE = native SessionStart hook (vibespace-hook.mjs; output MUST be the nested `{hookSpecificOutput:{hookEventName:'SessionStart', additionalContext}}` shape — top-level additionalContext is recorded as a hook_response record but NEVER reaches the model; the CLI binary even hints "Did you mean hookSpecificOutput"; fires on create AND resume, terminal AND chat). CODEX = context attached to the FIRST chat message (ws-handler; codex 0.142.x has NO session-start hook — its hook events are only user_prompt_submit/permission_request; `_taskCtxInjected` is in-memory, so a server restart or resume re-injects once — idempotent by content). `<vibespace-task-context>` is in the chat notification-tag list (renders as dim collapsible card). TASK.md: TaskManager writes `<contextDir>/.vibespace/TASK.md` on every change + boot (program-only writer, content-compare guard, atomic)
