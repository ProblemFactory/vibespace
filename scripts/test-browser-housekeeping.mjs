...
const hk = R.create({ dataDir: DATA, homeDir: HOME, keeper: stubKeeper, bridge, serverSetting: (k) => settings[k], broadcast: (m) => bcast.push(m), log: quiet, now, execFileImpl: fakeDu, sweepEveryMs: 0 });
{
  ...
  const ve = await hk.housekeeping();
  ok(ve.ephemeralBrowsers.length === 1 && ve.ephemeralBrowsers[0].browserKey === KEY_A && ve.ephemeralBrowsers[0].state === 'ready' && !ve.profiles.some((r) => r.id === 'bp-000000e1'), '…and passes the keeper\'s rows through as `ephemeralBrowsers` (the panel\'s "Ephemeral browsers" section) — none of them a profile row');
  delete stubKeeper.ephemerals;
...
console.log('— ④ the routes: the session-or-key match, the frame, PATCH, housekeeping, host refused');
{
  ...
  const l1 = await call('GET', `/api/browser/actions?sessionId=sess-1`);
  ok(l1.status === 200 && l1.body.entries.length === 5 && l1.body.browserKey === KEY_A && l1.body.traceOn === true, 'GET /api/browser/actions?sessionId= answers that session\'s entries (all scopes)');
  const l2 = await call('GET', `/api/browser/actions?sessionId=sess-resumed`);
  ...
  const l3 = await call('GET', `/api/browser/actions?sessionId=sess-unknown`);
  ...
  const lc = await call('GET', `/api/browser/actions?conversation=conv-a`);
  ...
  ok((await call('GET', `/api/browser/actions?conversation=conv-unknown`)).body.entries.length === 0 && (await call('GET', `/api/browser/actions?conversation=conv-unknown`)).body.browserKey === null, 'an unbound conversation: empty, key null');
  const lk = await call('GET', `/api/browser/actions?browserKey=${KEY_A}`);
  ok(lk.status === 200 && lk.body.entries.length === 5 && lk.body.keyFrom === 'given', 'a browserKey given outright matches too (keyFrom: given)');
  ok((await call('GET', `/api/browser/actions?browserKey=nope`)).status === 400 && (await call('GET', `/api/browser/actions?sessionId=sess-1&browserKey=nope`)).status === 400, 'a malformed browserKey is 400 even beside a good sessionId');
  const lp = await call('GET', `/api/browser/actions?sessionId=sess-1&conversation=conv-a`);
  ...
  const l4 = await call('GET', `/api/browser/actions?sessionId=sess-1&from=${first.at}&to=${first.at}&limit=1`);
  ok(l4.status === 200 && l4.body.entries.length === 1 && l4.body.entries[0].id === first.id, 'the window and the limit apply');
  ok((await call('GET', `/api/browser/actions?sessionId=sess-1&profile=${T.EPHEMERAL_SCOPE}`)).body.entries.length === 0 && (await call('GET', `/api/browser/actions?sessionId=sess-1&profile=nope`)).status === 400 && (await call('GET', `/api/browser/actions`)).status === 400, 'profile = ephemeral | id | empty; a bad one and a missing sessionId are 400');
  ok((await call('GET', `/api/browser/actions?sessionId=sess-1&host=h1`)).status === 400 && (await call('GET', `/api/browser/actions?sessionId=sess-1&host=h1`)).body.code === 'unsupported-host', 'host is refused by name');
  const one = await call('GET', `/api/browser/actions/${first.id}`);
  ok(one.status === 200 && one.body.entry.id === first.id && (await call('GET', `/api/browser/actions/tr-000000000000`)).status === 404 && (await call('GET', `/api/browser/actions/x`)).status === 400, 'one entry; unknown 404; a bad id 400');
  const fr = await call('GET', `/api/browser/actions/${first.id}/frame/before`);
  ...
  ok((await call('GET', `/api/browser/actions/${first.id}/frame/middle`)).status === 400 && (await call('GET', `/api/browser/actions/tr-000000000000/frame/after`)).status === 404, 'which ∈ {before, after}; a gone entry is 404');
...
console.log('— ⑥ the client half: the PURE card helpers, the wiring pins, the i18n census');
{
  ...
  ok(T.frameUrl('tr-0123456789ab', 'before') === '/api/browser/actions/tr-0123456789ab/frame/before' && T.frameUrl('tr-0123456789ab', 'middle') === '' && T.frameUrl('../x', 'after') === '', 'frameUrl: only an entry id, only before|after (drawn through .src, never markup)');
  ...
  ok(/registerWindowType\(\{\s*type: 'browser-profiles', label: 'Agent browser', singleton: true/.test(tv) && /const PANEL_TYPE = 'browser-profiles'/.test(tv) && /action: 'openBrowserProfiles'/.test(tv), 'the panel is a singleton window type with a replayable openSpec action (label Agent browser — design-browser-faces direction B; id + action unchanged)');
  ok(/app\.wm\.createWindow\(\{ title: t\('Agent browser'\), type: PANEL_TYPE, syncId, openSpec: \{ action: 'openBrowserProfiles' \}/.test(tv) && !/t\('Browser profiles'\)/.test(tv), "the window's title is t('Agent browser') (the agent's face, named for who drives it); the old 'Browser profiles' title is gone");
  ok(/registerMenuItem\(\{ menu: 'gear', parent: 'tools', order: 35/.test(tv) && /label: \(\) => t\('Agent browser…'\)/.test(tv) && /icon: UI_ICONS\.browserLive\b/.test(tv) && /when: \(c\) => !!c\.app\._browserProfiles/.test(tv), 'the ⚙ row (Agent browser…, the browser-live glyph) files under Tools at 35 (between Desktop apps 30 and Plugins 40), gated on the profile digest');
  ...
  // takeover C3 (design-browser-takeover §5.3): the "Ephemeral browsers" section — the record, whose conversation, its state, Stop
  ok(/const ephs = section\(t\('Ephemeral browsers'\)/.test(tv) && /v\.ephemeralBrowsers/.test(tv) && /ephs\.appendChild\(ephemeralRow\(e\)\)/.test(tv), 'the panel renders an "Ephemeral browsers" section from `ephemeralBrowsers`, one row each');
  const erow = tv.slice(tv.indexOf('  function ephemeralRow(e) {'), tv.indexOf('  function orphanRow(o) {'));
  ok(/bprof-ephemeral-browser/.test(erow) && /e\.browserKey/.test(erow) && /ephemeralStateText\(e\)/.test(erow) && /\/api\/browser\/profiles\/\$\{encodeURIComponent\(String\(e\.profileId \|\| ''\)\)\}\/stop/.test(erow) && /stop\.disabled = !e\.live/.test(erow), 'each row: the record, the conversation (its key), the state in words, and Stop through the profile stop route (disabled when not running)');
  ok(!/innerHTML/.test(erow) && /bprof-row\.bprof-ephemeral-browser \{ grid-template-columns/.test(read('public/style.css')), 'the row is built with textContent only, its grid has its own style rule');
  for (const [lang, d] of [['zh', zh], ['ja', ja]]) ok(['Ephemeral browsers', 'idled out', 'running (adopted)', 'Could not stop the browser', 'None — no conversation has browsed without a profile since its start.'].every((k2) => d.has(k2)), `the section's words have ${lang} entries`);
