...
const auditLines = () => { try { return fs.readFileSync(k.auditFile, 'utf8').trim().split('\n').filter(Boolean).map((l) => JSON.parse(l)); } catch { return []; } };
{
  ...
  // and the command runs with it — the binary never searches either directory
  {
    ...
    ok(cy.status === 0 && ly.length === 1 && ly[0].config === EPH_CFG && my && my.config === EPH_CFG && my.cwd === DIRTY, 'r3/r4: the keeper launched the rung-N browser with ITS config file (machine-ephemeral-<key>.json, marked) and the command from the dirty directory ran with the SAME file — named, never searched', { st: cy.status, ly, my, err: cy.stderr.slice(0, 300) });
