...
export const SUITES = [
  ...
  { name: 'test-hidden-view-suspend', tier: 'fast' }, // inc-mu6bfv1t-4drq: mobile / tab / minimized hiders suspend the ChatView like the desktop one (reason set + WindowManager.syncHiddenViews + wiring pins)
  ...
  { name: 'test-integrations-ui', tier: 'heavy', why: 'server + chrome: a real worktree server, a bundle build, a 375×667 page and a second client (~60s)' }, // design §14.5 in a browser: the setup block drawn ABOVE the fields with a copyable callback URL, Replace never reveals, the delegating dropdown resolves `prefer` and a saved choice overrides it, a PUT failure toasts, the deep link lands on the card, two clients see one change; ⑩ THE ACCOUNT MODEL (2026-09-22, c2): two offered credentials ⇒ the wizard's credential step with the chosen key in the connect body, "Add account…" mints a second account (two sections, distinct names, a chip each), re-authorize on the second posts to ITS id, one offered ⇒ the step is skipped silently, the chooser's caption reads "Default for new accounts"
