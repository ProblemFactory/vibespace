...
// ── wiring pins (the 2.331.0 lesson: a fix with an unstaged call site is dead) ──
{
  ...
  ok((wsh.match(/feedLive\(session, /g) || []).length === 1 && (ui.match(/feedLive\(session, /g) || []).length === 1 && (consumers.match(/feedLive\(session, /g) || []).length === 4 && (so.match(/feedLive\(session, /g) || []).length === 0, 'the six known live sites (chat-input echo, permission payload, stdout consumers ×4: codex / acp non-acp frames / acp records / claude — session-stdout itself feeds nothing since S5) are all gated');
