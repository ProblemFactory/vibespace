...
// B3. gear menu — a TREE FIXTURE over the 16-state matrix + the flatten CENSUS against the legacy flat list
{
  ...
  for (const [isMobile, _repoDir, _authEnabled, nPlugins] of cartesian([false, true], [null, '/repo'], [false, true], [0, 2])) {
    ...
    const app = { isMobile, _repoDir, _authEnabled, _fontSize: 14, _railEl: {}, _desktopAppsAvailable: true, _browserProfiles: { profiles: [] }, pluginClient: { contributedWindows: () => wins, open: () => {} } };
  ...
  const rows = menuItems('gear', { app: { isMobile: false, _repoDir: '/r', _authEnabled: true, _fontSize: 14, _railEl: {}, _desktopAppsAvailable: true, _browserProfiles: { profiles: [] } }, pop: {} });
  ...
  const app = new Proxy({ isMobile: false, _repoDir: '/r', _authEnabled: false, _fontSize: 14, _railEl: {}, _desktopAppsAvailable: true, _browserProfiles: { profiles: [] }, _customize: { enter: () => calls.push(['customize']) }, _settingsUI: { open: () => calls.push(['settingsOpen']) }, pluginClient: { contributedWindows: () => [{ pluginId: 'p', windowId: 'w', title: 'PW' }], open: (p, w) => calls.push(['pluginOpen', p, w]) } }, { get: (t, k) => (k in t ? t[k] : (...a) => calls.push([k, ...a])) });
