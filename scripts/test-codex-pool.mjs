...
// ── engine + wiring pins ──
{
  ...
  ok('codex pools are COLD-ONLY in the eval (hot needs the P3 re-read proof)', /isCodexPool = \(a\.backend \|\| 'claude'\) === 'codex';\s*\n\s*const hot = !!a\.hot && !isCodexPool;/.test(eng));
  ok('sealed orders + plan-C per-session links stay claude-only', /if \(!isCodexPool\) pushSealedOrders/.test(eng) && /if \(isCodexPool\) break; \/\/ plan-C/.test(eng));
  ...
  ok('the codex stdout pipeline feeds the engine (rate_limits_updated + task_failed)', /rate_limits_updated' \|\| msg\.payload\?\.type === 'task_failed'\)/.test(ss) && /recordCodexQuotaSignal\?\.\(session, msg\.payload\)/.test(ss));
