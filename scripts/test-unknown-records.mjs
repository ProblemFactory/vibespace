...
console.log('§6 the routed names (design-unknown-records, 2026-09-21): a routed name NO LONGER renders the red card — each has its own surface');
{
  ...
  // 2.369.147 (owner: a running Workflow card read 已完成 while the same run sat in the popup as an unclickable harness row):
  // the set names t-one AGAIN ⇒ the soft close was a transient drop — the card REOPENS; dropped again ⇒ closed again; progress ⇒ alive
  bg.processLive({ type: 'system', subtype: 'background_tasks_changed', tasks: [{ task_id: 't-one', task_type: 'local_agent', description: 'one' }, { task_id: 't-two', task_type: 'local_agent', description: 'two' }], uuid: 'bt2', session_id: 's' });
  ok('background_tasks_changed naming t-one again ⇒ its level-closed card REOPENS (running, closedBy gone) — the set is stronger evidence than the guess', one?.taskInfo?.status === 'running' && one.taskInfo.closedBy === undefined);
  bg.processLive({ type: 'system', subtype: 'background_tasks_changed', tasks: [{ task_id: 't-two', task_type: 'local_agent', description: 'two' }], uuid: 'bt3', session_id: 's' });
