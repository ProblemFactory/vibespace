...
const autoResume = require('./src/server/auto-resume.js').create({ dataDir: path.join(__dirname, 'data'), activeSessions, serverSetting, beforeFire: (id, s) => { try { return beforeAutoResumeFire(id, s); } catch (e) { console.warn('[auto-resume] pre-fire gate threw — refusing the continue (fail closed):', e && e.message); return false; } }, fireIdentity: (id, s) => { try { return fireIdentityFor(s); } catch { return null; } }, authorizeSpend: (id, s, identity, o) => spendGuard.authorize({ reason: 'auto-resume', session: s, sessionId: id, sessionName: s && s.name, identity, hold: !!(o && o.hold) }), noteSpend: (id, s, identity, hold) => spendGuard.note({ reason: 'auto-resume', session: s, identity, hold }), releaseSpend: (id, s, hold) => spendGuard.release({ hold }), log: (...a) => console.log(...a), broadcast: (id, m) => { const s = activeSessions.get(id); if (s) broadcastToSession(s, id, m); }, notify: (id, s, text, extra) => { try { feedPeerCard(s, { fromName: 'VibeSpace', text, ...(extra && extra.resetCredit ? { resetCredit: extra.resetCredit } : {}) }); } catch { } }, resetCreditOffer: (id, s) => { try { return resetCreditOffer(s); } catch { return null; } }, sendToSession: (id, s, text, carried) => { try { const ad = adapterRegistry.get(s.backend); if (!ad || !s.pty || s.mode !== 'chat') return false; const { stdinPayload, userMsg } = ad.formatChatInput(text, Date.now() + '-auto'); s._isStreaming = true; s._machineInputAt = Date.now(); /* a turn nobody typed — prompt-context withholds next-turn group reports from it (§22) */ s.pty.write(stdinPayload + '\n'); if (userMsg) { userMsg.originKind = 'auto-resume'; if (userMsg.payload) userMsg.payload.webui_origin = 'auto-resume'; const note = carried && typeof carried.note === 'string' && carried.note ? carried.note.slice(0, 200) : null; if (note) { userMsg.originNote = note; if (userMsg.payload) userMsg.payload.webui_origin_note = note; } s.buffer = (s.buffer + JSON.stringify(userMsg) + '\n').slice(-500000); feedLive(s, userMsg); } return true; } catch (e) { console.warn('[auto-resume] send failed:', e.message); return false; } } }); // continue a limited session when its quota resets ⇒ src/server/auto-resume.js
autoResume.start();
...
const vnc = new VncManager({ dataDir: path.join(__dirname, 'data'), stopOnShutdown: throwawayRoot }); // a throwaway server stops the Xvnc IT started at shutdown; an installed one's desktop outlives a restart (src/vnc.js)
...
const { desktopStream, windowEngine, boot: bootWindowLeases, shutdown: shutdownWindowLeases } = require('./src/server/window-live-wiring.js').install({
  ...
  netemEnabled: process.env.VIBESPACE_DESKTOP_NETEM === '1', access: desktopAccess, deliver, groupsOf: (s, id) => tasks.groupsForSession({ sessionKey: sessionStatusKey(s, id), cwd: s.cwd, initialGroupId: s._initialGroupId }).map((g) => g.id) }); // P8-2 D21 (c) validation slice: a dev-only latency/bit-rate injector on the xpra relay, never on by default; lane E: the gated ladder (the window request's wake) + Task Group membership asked at verb time (reach)
